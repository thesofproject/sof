...
#define SPIPE_HOST(scomp, hno_irq, hdmac, hchan, hconfig) \
	{.comp = scomp, .no_irq = hno_irq, \
	 .config.hdr.size = sizeof(struct sof_ipc_comp_config)}
...
#define SPIPE_MIX(scomp) \
	{.comp = scomp, \
	 .config.hdr.size = sizeof(struct sof_ipc_comp_config)}
#define SPIPE_SRC(scomp) \
	{.comp = scomp, \
	 .config.hdr.size = sizeof(struct sof_ipc_comp_config)}
#define SPIPE_TONE(scomp) \
	{.comp = scomp, \
	 .config.hdr.size = sizeof(struct sof_ipc_comp_config)}
 ...
 */
#define SPIPE_PIPE(cid, pid, pcore, pperiod, ppriority) \
	{.comp_id = cid, .pipeline_id = pid, .core = pcore, .period = pperiod, \
	.priority = ppriority}
...
static struct sof_ipc_buffer buffer0[] = {
	...
	SPIPE_BUFFER(16, HOST_PERIOD_SIZE * 2),

	/* B1 - LL Playback - PCM 1 - Host2 -> Volume3 */
	SPIPE_BUFFER(17, HOST_PERIOD_SIZE * 2),

	/* B2 Volume1 -> Mixer4 */
	SPIPE_BUFFER(18, INT_PERIOD_SIZE * 1),

	/* B3  Volume3 -> Mixer4 */
	SPIPE_BUFFER(19, INT_PERIOD_SIZE * 1),

	/* B4 Mixer4 -> Volume 5 */
	SPIPE_BUFFER(20, INT_PERIOD_SIZE * 1),

	/* B5 - DAI Playback - Volume5 -> DAI6 */
	SPIPE_BUFFER(21, DAI_PERIOD_SIZE * 2),

	/* B6 - DAI Capture - DAI7 - > Volume8 */
	SPIPE_BUFFER(22, DAI_PERIOD_SIZE * 2),

	/* B7 - PCM0 - Capture LL - Volume8 -> Host9 */
	SPIPE_BUFFER(23, HOST_PERIOD_SIZE * 1),
};
...
static struct sof_ipc_buffer buffer1[] = {
	...
	SPIPE_BUFFER(24, HOST_PERIOD_SIZE * 16),

	/* B9  SRC11 -> Volume12 */
	SPIPE_BUFFER(25, INT_PERIOD_SIZE * 2),
...
static struct sof_ipc_buffer buffer2[] = {
	/* B11 - tone13 -> SRC14 */
	SPIPE_BUFFER(27, HOST_PERIOD_SIZE * 16),

	/* B12  SRC14 -> Volume15 */
	SPIPE_BUFFER(28, INT_PERIOD_SIZE * 2),
...
static struct sof_ipc_pipe_comp_connect c_connect0[] = {
	SPIPE_COMP_CONNECT(0, 16), /* Host0 -> B0 ->  Volume1 */
	SPIPE_COMP_CONNECT(16, 1), /* Host0 -> B0 ->  Volume1 */
	SPIPE_COMP_CONNECT(2, 17), /* Host2 -> B1 ->  Volume3 */
	SPIPE_COMP_CONNECT(17, 3), /* Host2 -> B1 ->  Volume3 */
	SPIPE_COMP_CONNECT(1, 18), /* Volume1 -> B2 -> Mixer4 */
	SPIPE_COMP_CONNECT(18, 4), /* Volume1 -> B2 -> Mixer4 */
	SPIPE_COMP_CONNECT(3, 19), /* Volume3 -> B3 -> Mixer4 */
	SPIPE_COMP_CONNECT(19, 4), /* Volume3 -> B3 -> Mixer4 */
	SPIPE_COMP_CONNECT(4, 20), /* Mixer4 -> B4 -> Volume5 */
	SPIPE_COMP_CONNECT(20, 5), /* Mixer4 -> B4 -> Volume5 */
	SPIPE_COMP_CONNECT(5, 21), /* Volume5 -> B5 -> DAI6 */
	SPIPE_COMP_CONNECT(21, 6), /* Volume5 -> B5 -> DAI6 */
	SPIPE_COMP_CONNECT(7, 22), /* DAI7 -> B6 -> Volume8 */
	SPIPE_COMP_CONNECT(22, 8), /* DAI7 -> B6 -> Volume8 */
	SPIPE_COMP_CONNECT(8, 23), /* Volume8 -> B7 -> host9 */
	SPIPE_COMP_CONNECT(23, 9), /* Volume8 -> B7 -> host9 */
};
...
struct sof_ipc_pipe_new pipeline[] = {
	SPIPE_PIPE(30, 0, 0, 1000, SOF_TASK_PRI_HIGH),/* high pri - 1ms period */
