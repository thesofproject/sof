...
extern void sys_comp_module_level_multiplier_interface_init(void);
...
SOF_DEFINE_UUID("level_multiplier_test", level_multiplier_test_uuid,
		0x30397456, 0x4661, 0x4644,
		0x97, 0xe5, 0x39, 0xa9, 0xe5, 0xab, 0x17, 0x78);

struct custom_ipc4_config_level_multi {
	struct ipc4_base_module_cfg base;
} __attribute__((packed, aligned(8)));

/* Test level multiplier initialization */
static struct comp_dev *test_level_multiplier_create(void)
{
	struct comp_dev *comp;
	struct comp_ipc_config ipc_config;
	struct ipc_config_process spec;
	struct custom_ipc4_config_level_multi init_data;
	...
	list_for_item(clist, &comp_drivers_get()->list) {
		...
		if (!memcmp(info->drv->uid, &level_multiplier_test_uuid, sizeof(struct sof_uuid))) {
			drv = info->drv;
			break;
		}
	}
	zassert_not_null(drv, "Driver for level_multiplier not found");
...
/* Test level_multiplier initialization */
ZTEST(audio_level_multiplier, test_level_multiplier_init)
{
	struct comp_dev *comp = test_level_multiplier_create();
...
ZTEST(audio_level_multiplier, test_level_multiplier_config)
{
	struct comp_dev *comp = test_level_multiplier_create();
...
ZTEST(audio_level_multiplier, test_level_multiplier_process)
{
	struct comp_dev *comp = test_level_multiplier_create();
...
ZTEST_SUITE(audio_level_multiplier, NULL, setup, NULL, NULL, NULL);
