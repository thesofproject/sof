...
static struct comp_dev *test_stft_process_create(void)
{
...
/* Test stft_process initialization */
ZTEST(audio_stft_process, test_stft_process_init)
{
	struct comp_dev *comp = test_stft_process_create();
...
ZTEST(audio_stft_process, test_stft_process_config)
{
	struct comp_dev *comp = test_stft_process_create();
...
ZTEST(audio_stft_process, test_stft_process_process)
{
	struct comp_dev *comp = test_stft_process_create();
