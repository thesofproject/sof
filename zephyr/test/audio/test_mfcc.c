...
static struct comp_dev *test_mfcc_create(void)
{
...
{
	struct comp_dev *comp = test_mfcc_create();
...
ZTEST(audio_mfcc, test_mfcc_config)
{
	struct comp_dev *comp = test_mfcc_create();
	...
	if (comp->drv->ops.set_large_config) {
		...
		ret = comp->drv->ops.set_large_config(comp, 0, true, true,
			sizeof(mfcc_init_data), (uint8_t *)&mfcc_init_data);
...
ZTEST(audio_mfcc, test_mfcc_process)
{
	struct comp_dev *comp = test_mfcc_create();
	int ret = 0;
