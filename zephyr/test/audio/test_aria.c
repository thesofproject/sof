...
static struct comp_dev *test_aria_create(void)
{
	...
	aria_init_data.base.audio_fmt.valid_bit_depth = 24;
...
{
	struct comp_dev *comp = test_aria_create();
...
ZTEST(audio_aria, test_aria_config)
{
	struct comp_dev *comp = test_aria_create();
	zassert_not_null(comp, "ARIA allocation failed");
	...
	if (comp->drv->ops.set_large_config) {
		ret = comp->drv->ops.set_large_config(comp, 1, true, true,
			sizeof(attenuation), (uint8_t *)&attenuation);
	}

	comp->drv->ops.free(comp);
}

ZTEST(audio_aria, test_aria_process)
{
	struct comp_dev *comp = test_aria_create();
	zassert_not_null(comp, "ARIA allocation failed");
	...
	params.sample_valid_bytes = 3; /* ARIA requires S24_4LE */
