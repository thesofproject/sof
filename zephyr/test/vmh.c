...
static void test_vmh_alloc_multiple_times(bool allocating_continuously)
{
	...
	vmh_init_heap(NULL, MEM_REG_ATTR_SHARED_HEAP, allocating_continuously);
...
{
	test_vmh_init_and_free_heap(MEM_REG_ATTR_SHARED_HEAP, NULL, 0, false, true);
	...
	test_vmh_init_and_free_heap(MEM_REG_ATTR_SHARED_HEAP, &config, 0, false, true);
}
...
static void test_alloc_on_configured_heap(bool allocating_continuously)
{
	...
	struct vmh_heap *heap =
	vmh_init_heap(&config, MEM_REG_ATTR_SHARED_HEAP, allocating_continuously);
