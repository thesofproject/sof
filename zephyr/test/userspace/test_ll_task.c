...
static void test_ll_task_free(struct task *task)
{
 ...
 */
static void pipeline_two_components(void)
{
	...
	ctx->alloc = sof_heap_alloc(heap, SOF_MEM_FLAG_USER, sizeof(struct mod_alloc_ctx), 0);
	ctx->alloc->heap = heap;
	ctx->alloc->vreg = NULL;
	ctx->heap = heap;
	ctx->ipc = ipc_get();
	...
	ctx->buf = buffer_alloc(ctx->alloc, 384, 0, 0, false);
	...
	user_ll_grant_access(&ppl_user_thread, ctx->ipc_pipe->core);
