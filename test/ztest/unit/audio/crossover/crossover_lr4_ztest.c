 ...
 */
ZTEST(crossover_lr4_suite, test_lr4_identity_passthrough)
{
	...
	for (int i = 0; i < SIGNAL_LEN; i++) {
		...
		zassert_equal(x, y, "identity mismatch at sample %d", i);
	}
}
 ...
 */
ZTEST(crossover_lr4_suite, test_lr4_lp_dc_pass_hf_block)
{
	struct iir_state_df1 lr4;
	int32_t coef[LR4_NCOEF];
	int32_t delay[LR4_NDELAY];
	...
	zassert_true(fabs(rms_out_dc - rms_in) / rms_in < 0.01,
		     "LP DC response is outside tolerance");
	...
	zassert_true(rms_out_hf < 0.01 * amp / (1u << 31),
		     "LP does not sufficiently attenuate high frequency");
}
 ...
 */
ZTEST(crossover_lr4_suite, test_lr4_hp_dc_block_hf_pass)
{
	struct iir_state_df1 lr4;
	int32_t coef[LR4_NCOEF];
	int32_t delay[LR4_NDELAY];
	...
	zassert_true(rms_out_dc < 0.01 * (double)x_dc / (1u << 31),
		     "HP does not sufficiently attenuate DC");
	...
	zassert_true(fabs(rms_out_hf - rms_in) / rms_in < 0.05,
		     "HP high-frequency response is outside tolerance");
}
 ...
 */
ZTEST(crossover_lr4_suite, test_lr4_crossover_equal_power_split)
{
	...
	zassert_true(fabs(rms_lp - rms_hp) / rms_in < 0.05,
		     "LP and HP levels are not symmetric at crossover");

	/* Each output must be at −6 dB (amplitude × 0.5) relative to input */
	zassert_true(fabs(rms_lp - expected) / rms_in < 0.05,
		     "LP level is outside expected crossover response");
	zassert_true(fabs(rms_hp - expected) / rms_in < 0.05,
		     "HP level is outside expected crossover response");
}
 ...
 */
ZTEST(crossover_lr4_suite, test_lr4_lp_frequency_selectivity)
{
	struct iir_state_df1 lr4;
	int32_t coef[LR4_NCOEF];
	int32_t delay[LR4_NDELAY];
	...
	zassert_true(rms_lf > 100.0 * rms_hf,
		     "LP does not provide the expected frequency selectivity");
}
 ...
 * The ratio of the two output RMS values must be at least 100 (40 dB).
 */
ZTEST(crossover_lr4_suite, test_lr4_hp_frequency_selectivity)
{
	struct iir_state_df1 lr4;
	int32_t coef[LR4_NCOEF];
	int32_t delay[LR4_NDELAY];
	...
	zassert_true(rms_hf > 100.0 * rms_lf,
		     "HP does not provide the expected frequency selectivity");
}
 ...
 */
ZTEST(crossover_lr4_suite, test_lr4_lp_hp_same_denominator)
{
	int32_t bq_lp[BIQUAD_NCOEF];
	int32_t bq_hp[BIQUAD_NCOEF];
	...
	zassert_equal(bq_lp[0], bq_hp[0], "LP and HP a2 coefficients differ");
	zassert_equal(bq_lp[1], bq_hp[1], "LP and HP a1 coefficients differ");

	/* b-coefficients must differ */
	zassert_true(bq_lp[3] != bq_hp[3], "LP and HP b1 coefficients match");
}
...
ZTEST_SUITE(crossover_lr4_suite, NULL, NULL, NULL, NULL, NULL);
