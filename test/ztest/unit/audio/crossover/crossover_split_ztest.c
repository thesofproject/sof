				 ...
				 double fc_lo, double fc_hi)
{
	...
	case 4:
		 ...
		 */
		{
			double fc[CROSSOVER_MAX_LR4] = {
				fc_lo, (fc_lo + fc_hi) / 2.0, fc_hi
			};
			...
			for (i = 0; i < CROSSOVER_MAX_LR4; i++) {
				...
				compute_biquad_hp(FS, fc[i], bq_hp);
 ...
 */
ZTEST(crossover_split_suite, test_find_split_func_invalid)
{
	zassert_is_null(crossover_find_split_func(0), "0 sinks must be invalid");
	zassert_is_null(crossover_find_split_func(1), "1 sink must be invalid");
	zassert_is_null(crossover_find_split_func(5), "5 sinks must be invalid");
}

/**
 * @brief Test 2 – crossover_find_split_func returns a valid function for 2, 3, 4.
 */
ZTEST(crossover_split_suite, test_find_split_func_valid)
{
	zassert_not_null(crossover_find_split_func(2), "2-way split is unavailable");
	zassert_not_null(crossover_find_split_func(3), "3-way split is unavailable");
	zassert_not_null(crossover_find_split_func(4), "4-way split is unavailable");
}

/**
 * @brief Test 3 – 2-way split produces non-zero outputs for a non-zero input.
 */
ZTEST(crossover_split_suite, test_split_2way_nonzero_output)
{
	...
	zassert_true(out[0] != 0 || out[1] != 0,
		     "2-way split produced no output for a non-zero input");
}

/**
 * @brief Test 4 – 2-way split produces zero outputs for a zero input.
 */
ZTEST(crossover_split_suite, test_split_2way_zero_input)
{
	struct crossover_state ch_state;
	struct channel_storage storage;
	...
	zassert_equal(out[0], 0, "2-way LP output is non-zero for zero input");
	zassert_equal(out[1], 0, "2-way HP output is non-zero for zero input");
}
 ...
 */
ZTEST(crossover_split_suite, test_split_2way_lp_frequency_selectivity)
{
	struct crossover_state ch_state;
	struct channel_storage storage;
	...
	zassert_true(lp_lf > 100.0 * lp_hf,
		     "2-way LP band lacks low-frequency selectivity");

	/* HP band: at least 40 dB stronger at 10 kHz than at 100 Hz */
	zassert_true(hp_hf > 100.0 * hp_lf,
		     "2-way HP band lacks high-frequency selectivity");
}
 ...
 */
ZTEST(crossover_split_suite, test_split_2way_band_dominance)
{
	struct crossover_state ch_state;
	struct channel_storage storage;
	double amp = 0.5 * INT32_MAX;
	...
	zassert_true(lp_50 > 100.0 * hp_50,
		     "2-way LP band does not dominate at 50 Hz");
	/* HP must dominate at 20 kHz by at least 40 dB */
	zassert_true(hp_20k > 100.0 * lp_20k,
		     "2-way HP band does not dominate at 20 kHz");
}
 ...
 */
ZTEST(crossover_split_suite, test_split_3way_all_bands_nonzero)
{
	struct crossover_state ch_state;
	struct channel_storage storage;
	double amp = 0.5 * INT32_MAX;
	...
	printf("3-way non-zero counts: band0=%d  band1=%d  band2=%d\n",
	       acc[0], acc[1], acc[2]);
	zassert_true(acc[0] > 0, "3-way band 0 is always zero");
	zassert_true(acc[1] > 0, "3-way band 1 is always zero");
	zassert_true(acc[2] > 0, "3-way band 2 is always zero");
}
 ...
 */
ZTEST(crossover_split_suite, test_split_3way_band_dominance)
{
	struct crossover_state ch_state;
	struct channel_storage storage;
	double amp = 0.5 * INT32_MAX;
	...
	zassert_true(rms3[0][0] > 100.0 * rms3[0][1],
		     "3-way bass band does not dominate mid band at 50 Hz");
	zassert_true(rms3[0][0] > 100.0 * rms3[0][2],
		     "3-way bass band does not dominate treble band at 50 Hz");

	/* 2 kHz → mid band dominates */
	zassert_true(rms3[1][1] > 10.0 * rms3[1][0],
		     "3-way mid band does not dominate bass band at 2 kHz");
	zassert_true(rms3[1][1] > 10.0 * rms3[1][2],
		     "3-way mid band does not dominate treble band at 2 kHz");

	/* 20 kHz → treble band dominates */
	zassert_true(rms3[2][2] > 100.0 * rms3[2][0],
		     "3-way treble band does not dominate bass band at 20 kHz");
	zassert_true(rms3[2][2] > 100.0 * rms3[2][1],
		     "3-way treble band does not dominate mid band at 20 kHz");
}

/**
 * @brief Test 9 – 4-way split produces non-zero output in all four bands.
 */
ZTEST(crossover_split_suite, test_split_4way_all_bands_nonzero)
{
	struct crossover_state ch_state;
	struct channel_storage storage;
	double amp = 0.5 * INT32_MAX;
	...
	zassert_true(acc[0] > 0, "4-way band 0 is always zero");
	zassert_true(acc[1] > 0, "4-way band 1 is always zero");
	zassert_true(acc[2] > 0, "4-way band 2 is always zero");
	zassert_true(acc[3] > 0, "4-way band 3 is always zero");
}
 ...
 */
ZTEST(crossover_split_suite, test_split_4way_band_dominance)
{
	struct crossover_state ch_state;
	struct channel_storage storage;
	double amp = 0.5 * INT32_MAX;
	...
	zassert_true(rms_50[0] > 100.0 * rms_50[1],
		     "4-way band 0 does not dominate band 1 at 50 Hz");
	zassert_true(rms_50[0] > 100.0 * rms_50[2],
		     "4-way band 0 does not dominate band 2 at 50 Hz");
	zassert_true(rms_50[0] > 100.0 * rms_50[3],
		     "4-way band 0 does not dominate band 3 at 50 Hz");

	/* 20 kHz → treble dominates */
	zassert_true(rms_20k[3] > 30.0 * rms_20k[0],
		     "4-way band 3 does not dominate band 0 at 20 kHz");
	zassert_true(rms_20k[3] > 30.0 * rms_20k[1],
		     "4-way band 3 does not dominate band 1 at 20 kHz");
	zassert_true(rms_20k[3] > 30.0 * rms_20k[2],
		     "4-way band 3 does not dominate band 2 at 20 kHz");
}
...
ZTEST_SUITE(crossover_split_suite, NULL, NULL, NULL, NULL, NULL);
