...
namespace intel_adsp
{
    ...
    /*! \brief Fixed-size container of continous elements whose size is specified at runtime.
     *
     * This container is very similar to the C-array except that its fixed-size is specified at runtime.
    ...
    struct FixedArray
    {
         ...
         *  \note If index is out of range (i.e. is equal or greater than value returned by GetLength()) the returned value is mal-formed.
         *  \remarks Return is by value rather than reference to prevent client code from working with dangling reference.
        ...
        { length = MIN(length, length_); for(int i = 0 ; i < length ; i++) array[i] = array_[i]; }
