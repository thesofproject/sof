					...
					struct sof_mod_hdr *module)
{
	...
	FILE *out_image_fptr = fopen("sof-acp7x-code.bin", "wb+");
	FILE *out_data_fptr = fopen("sof-acp7x-data.bin", "wb+");
	__u32 prev_size = 0;
	u32 prev_offset = 0;
	...
	for (count = 0; count < module->num_blocks; count++) {
		...
		if (count < module->num_blocks - 1 && !(block->offset == 0)
		    && block->type == prev_type) {
			switch (prev_type) {
			case SOF_FW_BLK_TYPE_IRAM:
...
int main(void)
{
	int ret;
	u32 payload_offset;
	FILE *inpt_fw_fptr = fopen("sof-acp7x.ri", "rb+");
	...
	// payload_offset <==> sdev->basefw.payload_offset
		...
		ret = ext_man_size;
		printf("dev_err : error: firmware sof-acp_7_x.ri contains unsupported or \
