...
static struct endpoint_hw_config *alsa_get_hw_config(struct comp_dev *dev)
{
	...
	for (i = 0; i < glb->num_ep_configs; i++) {
		ep_hw = &glb->ep_config[i];

		if (ep_hw->pipeline == cd->pipeline_id)
			return ep_hw;
	}
				 ...
				 const void *spec)
{
	...
	if (config->type != SOF_COMP_DAI) {
		comp_err(dev, "error: invalid comp type %d for ALSA PCM\n",
			config->type);
		return NULL;
	}
	...
	cd->pipeline_id = config->pipeline_id;

	err = plug_shm_init(&cd->pcm, _sp->topology_name, "pcm", cd->pipeline_id);
	if (err < 0)
		goto error;
...
static int set_params(struct comp_dev *dev)
{
	...
	err = snd_pcm_sw_params_set_stop_threshold(cd->handle, cd->sw_params, cd->period_frames / 4);
	if (err < 0) {
		 ...
		 int dir)
{
	...
	if (!ep_hw) {
		comp_err(dev, "error: failed to get hw config for pipeline %d\n",
			cd->pipeline_id);
		return -EINVAL;
	}
	cd->ep_hw = ep_hw;
	...
	if (!strncmp(cd->ep_hw->dev_name, "default", sizeof(cd->ep_hw->dev_name))) {
		snprintf(pcm_name, sizeof(pcm_name), "%s",
				cd->ep_hw->card_name);
	} else {
