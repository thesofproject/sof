...
static int parse_item(snd_config_t *cfg, const char *id,
			struct plug_config *dest_cfg)
{
	void *dest = dest_cfg;
	int i;
 ...
 * tplg:pcm_pipe:[dai_pipe:card:dev:config[dai_pipe:card:dev:config]...]
 */
static int parse_client_cmdline(snd_sof_plug_t *plug, char *cmdline)
{
	struct plug_cmdline_item *cmd_item;
	char *tplg, *next, *card, *dev, *config, *pipe;
	char tplg_file[128];
	int ret;
	int i;
	...
	/* now convert to filename */
	ret = snprintf(tplg_file, sizeof(tplg_file), "sof-%s.tplg", tplg);
	if (ret < 0) {
	...
	/* get PCM pipeline ID */
	tplg = strtok_r(next, ":", &next);
	if (!tplg) {
		SNDERR("invalid cmdline, cant find pipeline %s", tplg);
		return -EINVAL;
	}
	plug->tplg_pipeline = atoi(tplg);

	fprintf(stdout, "Parsing cmd line\n");

	/* now iterate though config tuples */
	do {
		cmd_item = &plug->cmdline[plug->num_cmdline];
		pipe = strtok_r(next, ":", &next);
	...
	} while (pipe && card && dev && config && plug->num_cmdline < PLUG_MAX_CONFIG);
		    ...
		    snd_config_t *conf)
{
	 ...
	 * i.e. aplay -Dsof:tplg:pipeline:[card:dev:config[card:dev:config]...]
	 */
