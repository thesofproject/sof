...
/*
 * :[pipe:card:dev:config[pipe:card:dev:config]...]
 */
...
static inline void *plug_ep_consume(struct plug_shm_endpoint *ep,
		unsigned bytes)
{
	ep->rtotal += bytes;
	ep->rpos += bytes;
...
static inline void *plug_ep_produce(struct plug_shm_endpoint *ep,
		unsigned bytes)
{
	ep->wtotal += bytes;
	ep->wpos += bytes;
...
long plug_timespec_delta_ns(struct timespec *before,
