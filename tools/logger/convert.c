...
static int fetch_entry(struct convert_config *config, uint32_t base_address,
	uint32_t data_offset, struct log_entry_header dma_log,
	uint64_t *last_timestamp)
{
	...
	ret = fread(entry.text, sizeof(char), entry.header.text_len,
		config->ldc_fd);
	...
	struct snd_sof_logs_header *snd)
{
	...
	while (!feof(config->in_fd)) {
		...
		if ((dma_log.log_entry_address < snd->base_address) ||
			 ...
			 */
			fseek(config->in_fd,
				-(sizeof(dma_log) - sizeof(uint32_t)),
				SEEK_CUR);
			continue;
		}
...
int convert(struct convert_config *config)
{
	...
	if (strncmp((char *) snd.sig, SND_SOF_LOGS_SIG,
		SND_SOF_LOGS_SIG_SIZE)) {
	...
	if (config->version_fd) {
		...
		ret = memcmp(&ver, &snd.version,
			sizeof(struct sof_ipc_fw_version));
		if (ret) {
