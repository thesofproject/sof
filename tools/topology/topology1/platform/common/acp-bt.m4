...
dnl ACPBT_VIRTUAL_CONFIG(format, mclk, bclk, fsync, tdm, esai_config_data)
define(`ACPBT_VIRTUAL_CONFIG',
`       format          "'$1`"'
`       '$2
`       '$3
`       '$4
...
dnl ACPBT_CONFIG_DATA(type, idx, rate, channel,i2s_tdm_mode)
#i2s_tdm_mode 1-> tdm mode, 0->i2s mode
define(`ACPBT_CONFIG_DATA',
...
dnl ACPBT_VIRTUAL_CONFIG_DATA(type, idx, rate, channel,i2s_tdm_mode)
#i2s_tdm_mode 1-> tdm mode, 0->i2s mode
define(`ACPBT_VIRTUAL_CONFIG_DATA',
`SectionVendorTuples."'N_DAI_CONFIG($1$2)`_tuples" {'
`       tokens "sof_acp_tokens"'
`       tuples."word" {'
`               SOF_TKN_AMD_ACP_RATE'		STR($3)
