...
# By default, use 100 Hz highpass response with +10 dB gain for 16khz
ifdef(`DMIC16KPROC_FILTER1', , `define(DMIC16KPROC_FILTER1, eq_iir_coef_highpass_100hz_10db_16khz.m4)')
...
# "EQ 0" has x source and 2 sink periods
...
P_GRAPH(pipe-vol-kfbm-capture, PIPELINE_ID,
	LIST(`		',
	`dapm(N_PCMC(PCM_ID), N_BUFFER(2))',
	`dapm(N_BUFFER(2), N_KPBM(0, PIPELINE_ID))',
	`dapm(N_KPBM(0, PIPELINE_ID), N_BUFFER(1))',
	`dapm(N_BUFFER(1), N_EQ_IIR(0, PIPELINE_ID))',
	`dapm(N_EQ_IIR(0, PIPELINE_ID), N_BUFFER(0))'))
