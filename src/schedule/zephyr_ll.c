...
struct zephyr_ll {
	...
	struct sof_umutex lock;			/* mutex for userspace */
#endif
	struct k_heap *heap;
};
 ...
 *
 * Uses the LL scheduler's own sof_umutex which is re-entrant, so
