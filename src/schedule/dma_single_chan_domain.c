...
{
	tr_info("dma_domain_notify_change()");
					       ...
					       void *arg)
{
	...
	tr_info("dma_single_chan_domain_irq_register()");
...
{
	tr_info("dma_single_chan_domain_irq_unregister()");
					   ...
					   void *arg)
{
	...
	tr_info("dma_single_chan_domain_register()");
	...
	if (data->channel) {
		...
		tr_info("dma_single_chan_domain_register(): lower period detected, registering again");
	...
	if (channel->period <= UINT_MAX)
		tr_info("dma_single_chan_domain_register(): registering on channel with period %u",
					...
					struct dma_domain_data *data)
{
	...
	tr_info("dma_domain_unregister_owner()");
					     ...
					     uint32_t num_tasks)
{
	...
	tr_info("dma_single_chan_domain_unregister()");
						       ...
						       int clk)
{
	...
	tr_info("dma_single_chan_domain_init(): num_dma %d, clk %d",
		num_dma, clk);
