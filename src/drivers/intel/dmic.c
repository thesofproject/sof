	...
	for (i = 0; i < DMIC_HW_CONTROLLERS; i++) {
		if (dmic_active_fifos == 0) {
			...
			ref = CIC_CONFIG_CIC_SHIFT(cfg->cic_shift + 8) |
				CIC_CONFIG_COMB_COUNT(cfg->mcic - 1);
			if ((val & (MASK(27, 24) | MASK(15, 8))) != ref) {
				dai_err(dai, "configure_registers(): Conflicting CIC_CONFIG %08x",
					val);
				return -EINVAL;
			}
			...
			if ((val & MASK(15, 8)) != ref) {
				dai_err(dai, "configure_registers(): Conflicting MIC_CONTROL %08x",
					val);
				return -EINVAL;
			}
		}
