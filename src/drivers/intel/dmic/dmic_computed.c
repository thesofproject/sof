 ...
 */
static void find_modes(struct dai *dai, struct decim_modes *modes, uint32_t fs, int di)
{
...
int dmic_get_hw_params_computed(struct dai *dai, struct sof_ipc_stream_params  *params, int dir)
{
	...
	params->rate = pdata->prm[di]->fifo_fs;
	params->buffer_fmt = 0;

	switch (pdata->prm[di]->num_pdm_active) {
	case 1:
		params->channels = 2;
		break;
	case 2:
...
static inline int ipm_helper1(struct dmic_pdata *pdata, int di)
{
	...
	for (i = 0; i < DMIC_HW_CONTROLLERS; i++) {
		if (pdata->prm[di]->pdm[i].enable_mic_a || pdata->prm[di]->pdm[i].enable_mic_b)
			pdm[i] = 1;
		else
			pdm[i] = 0;
	}
...
static inline void ipm_helper2(struct dmic_pdata *pdata, int source[], int *ipm, int di)
{
	int pdm[DMIC_HW_CONTROLLERS];
	int i;
	int n = 0;
 ...
 * swapped mono left.
 */
static int stereo_helper(struct dmic_pdata *pdata, int stereo[], int swap[])
{
	int cnt;
	int i;
	int swap_check;
	...
	for (i = 0; i < DMIC_HW_CONTROLLERS; i++) {
		cnt = 0;
		if (pdata->prm[0]->pdm[i].enable_mic_a || pdata->prm[1]->pdm[i].enable_mic_a)
			cnt++;

		if (pdata->prm[0]->pdm[i].enable_mic_b || pdata->prm[1]->pdm[i].enable_mic_b)
			cnt++;
		...
		swap[i] = (pdata->prm[0]->pdm[i].enable_mic_b ||
			pdata->prm[1]->pdm[i].enable_mic_b) && !cnt;

		/* Check that swap does not conflict with other DAI request */
		swap_check = pdata->prm[1]->pdm[i].enable_mic_a ||
			pdata->prm[0]->pdm[i].enable_mic_a;
...
static void of_helper(struct dmic_pdata *pdata, int *of0, int *of1)
{
	*of0 = (pdata->prm[0]->fifo_bits == 32) ? 2 : 0;

#if DMIC_HW_FIFOS > 1
	*of1 = (pdata->prm[1]->fifo_bits == 32) ? 2 : 0;
#else
	*of1 = 0;
#endif
}

static int configure_registers_computed(struct dai *dai, struct dmic_configuration *cfg)
{
	struct dmic_pdata *pdata = dai_get_drvdata(dai);
	...
	if (di == 0) {
		ipm_helper2(pdata, source, &ipm, 0);
		val = OUTCONTROL0_TIE(0) |
			OUTCONTROL0_SIP(0) |
			OUTCONTROL0_FINIT(1) |
			OUTCONTROL0_FCI(0) |
		...
		ipm_helper2(pdata, source, &ipm, 1);
		val = OUTCONTROL1_TIE(0) |
			OUTCONTROL1_SIP(0) |
			OUTCONTROL1_FINIT(1) |
			OUTCONTROL1_FCI(0) |
	 ...
	 */
	for (i = 0; i < DMIC_HW_CONTROLLERS; i++) {
		pdata->enable[i] = (pdata->prm[di]->pdm[i].enable_mic_b << 1) |
			pdata->prm[di]->pdm[i].enable_mic_a;
	}

	platform_shared_commit(pdata, sizeof(*pdata));

	ret = stereo_helper(pdata, stereo, swap);
	if (ret < 0) {
	...
	for (i = 0; i < DMIC_HW_CONTROLLERS; i++) {
		/* If any of the FIFOs is active check compatibility of new request. If
		 * no FIFOs running the shared resource (CIC) is set up.
		 */
		if (*(pdata->dmic_active_fifos))  {
			...
			val = CIC_CONTROL_SOFT_RESET(soft_reset) |
				...
				CIC_CONTROL_MIC_B_POLARITY(pdata->prm[di]->pdm[i].polarity_mic_a) |
				CIC_CONTROL_MIC_A_POLARITY(pdata->prm[di]->pdm[i].polarity_mic_b) |
			 ...
			 * processing active.
			 */
			edge = pdata->prm[di]->pdm[i].clk_edge;
			if (swap[i])
				edge = !edge;

			val = MIC_CONTROL_PDM_CLKDIV(cfg->clkdiv - 2) |
				MIC_CONTROL_PDM_SKEW(pdata->prm[di]->pdm[i].skew) |
...
int dmic_set_config_computed(struct dai *dai, struct sof_ipc_dai_config *config)
{
	...
	dai_info(dai, "dmic_set_config(), prm duty_min = %u, duty_max = %u",
		 prm->duty_min, prm->duty_max);
	dai_info(dai, "dmic_set_config(), prm fifo_fs = %u, fifo_bits = %u",
		 prm->fifo_fs, prm->fifo_bits);

	switch (prm->fifo_bits) {
	case 0:
	case 16:
	case 32:
		break;
	 ...
	 * shift values.
	 */
	find_modes(dai, &modes_a, dmic->prm[0]->fifo_fs, di);
	if (modes_a.num_of_modes == 0 && dmic->prm[0]->fifo_fs > 0) {
		dai_err(dai, "dmic_set_config(): No modes found found for FIFO A");
		return -EINVAL;
	}

	find_modes(dai, &modes_b, dmic->prm[1]->fifo_fs, di);
	if (modes_b.num_of_modes == 0 && dmic->prm[1]->fifo_fs > 0) {
	 ...
	 */
	ret = configure_registers_computed(dai, &cfg);
	if (ret < 0) {
