 ...
 * in use, it can be adjusted only with dividers. In order to change input,
 * the source should not be in use, that's why it's necessary to keep track
 * of BCLKs sources to know when it's safe to change shared input clock.
 */
					       ...
					       uint32_t *m, uint32_t *n)
{
	...
	mn_reg_write(0x0, (mn_reg_read(0x0) |
					  ...
					  uint32_t *scr_div)
{
	...
	for (i = 0; i <= MAX_SSP_FREQ_INDEX; i++) {
		...
		/* check if can be handled in M/N setup */
		...
		uint32_t *out_scr_div, bool *out_need_ecs)
{
	...
	*out_need_ecs = true;
	...
	if (ret >= 0) {
		...
		mn_reg_write(0x100 + dai_index * 0x8 + 0x0, m);
		mn_reg_write(0x100 + dai_index * 0x8 + 0x4, n);
	}
