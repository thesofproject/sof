...
int64_t platform_timer_set(struct timer *timer, uint64_t ticks)
{
	...
	/* Enable IRQ */
	shim_write(SHIM_DSPWCTCS, shim_read(SHIM_DSPWCTCS) | SHIM_DSPWCTCS_T0A);
...
void platform_timer_clear(struct timer *timer)
{
	...
	shim_write(SHIM_DSPWCTCS, shim_read(SHIM_DSPWCTCS) | SHIM_DSPWCTCS_T0T);
}
	...
	void (*handler)(void *arg), void *arg)
{
	...
	uint32_t dw_num = PLATFORM_TIMER_IRQ;
	uint32_t mask = 0x2;

	err = interrupt_register(PLATFORM_TIMER_INTERRUPT, handler, arg);
	if (err < 0)
		return err;

	/* enable timer interrupt */
	interrupt_enable(LVL_2_IRQ_NUMBER, arg);

	/* ACE IRQ controller */
	shim_write(SHIM_DSPWCTCS, shim_read(SHIM_DSPWCTCS) | SHIM_DSPWCTCS_T0TIE);
	shim_write(SHIM_DSPWCTCS, shim_read(SHIM_DSPWCTCS) | SHIM_DSPWCTCS_T0T);
	//controller interrupt:
	volatile uint16_t * const instancePtr = (uint16_t *)(IRQ_BASE + MS_REG_DxTTSIE_OFFSET);
