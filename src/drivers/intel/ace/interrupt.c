				    ...
				    uint32_t ilxmsd)
{
	...
	uint32_t status;
	int tries = LVL2_MAX_TRIES;
	k_spinlock_key_t key;

	volatile union DxINTIPPTR *ip_irq_add = (union DxINTIPPTR *) (0x78800+0x8);
	uint32_t ip_addr = ip_irq_add->part.ptr;

	volatile struct DW_ICTL_REGS *regs = (struct DW_ICTL_REGS *) ip_addr;
	status = regs->finalstatus_l;
	...
	for (;;) {
		...
		list_for_item(clist, &cascade->child[bit].list) {
			...
			// TODO: Temp workaround as ACE build has some problem with core mask.
			// int core = cpu_get_id();
