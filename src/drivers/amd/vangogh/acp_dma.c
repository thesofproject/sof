...
void amd_dma_reconfig(struct dma_chan_data *channel, uint32_t bytes)
{
	...
	if (channel->direction == DMA_DIR_HMEM_TO_LMEM) {
		...
		psrc_dscr[strt_idx].dest_addr = (dest | ACP_DRAM_ADDR_TRNS);
		...
		if (dma_cfg->rd_size + bytes > dma_cfg->sys_buff_size) {
			...
			psrc_dscr[strt_idx + 1].dest_addr = (dest1 | ACP_DRAM_ADDR_TRNS);
		...
		if (dma_cfg->wr_size + bytes > dma_cfg->size) {
			...
			psrc_dscr[strt_idx + 1].dest_addr =
				((dma_cfg->base & ACP_DRAM_ADDRESS_MASK) | ACP_DRAM_ADDR_TRNS);
		...
		psrc_dscr[strt_idx].src_addr = (src | ACP_DRAM_ADDR_TRNS);
		...
		if (dma_cfg->wr_size + bytes > dma_cfg->sys_buff_size) {
			...
			psrc_dscr[strt_idx + 1].src_addr = (src1 | ACP_DRAM_ADDR_TRNS);
			dma_config_descriptor(strt_idx, 2, psrc_dscr, pdest_dscr);
			dma_chan_reg_write(channel, ACP_DMA_DSCR_CNT_0, 2);
			dma_cfg->wr_size  = 0;
		}
...
{
	uint32_t dscr_cnt, dscr;
	uint32_t tc;
	uint16_t dscr_strt_idx;
	uint32_t *phy_off;
	...
	for (dscr = 0; dscr < dscr_cnt; dscr++) {
		if (dir == DMA_DIR_HMEM_TO_LMEM) {
			...
			src = (src & ACP_DRAM_ADDRESS_MASK);
			dma_config_dscr[dscr_strt_idx + dscr].src_addr = (src | ACP_DRAM_ADDR_TRNS);
			dma_config_dscr[dscr_strt_idx + dscr].trns_cnt.u32all = 0;
			dma_config_dscr[dscr_strt_idx + dscr].trns_cnt.bits.trns_cnt =
					sgelems->elems[dscr].size;
		}
