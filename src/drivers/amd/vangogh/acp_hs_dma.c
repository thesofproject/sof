				     ...
				     struct dma_sg_config *config)
{
	...
	if (config->direction ==  DMA_DIR_MEM_TO_DEV) {
		...
		io_reg_write((PU_REGISTER_BASE + ACP_HS_TX_FIFOSIZE), (uint32_t)(HS_FIFO_SIZE));

		/* Transmit RINGBUFFER Address and size*/
		config->elem_array.elems[0].src =
			(config->elem_array.elems[0].src & ACP_DRAM_ADDRESS_MASK);
		hs_buff_addr = (config->elem_array.elems[0].src | ACP_DRAM_ADDR_TRNS);
		io_reg_write((PU_REGISTER_BASE + ACP_HS_TX_RINGBUFADDR), hs_buff_addr);
		io_reg_write((PU_REGISTER_BASE + ACP_HS_TX_RINGBUFSIZE), hs_buff_size);
		...
		hs_fifo_addr = (uint32_t)(&pscratch_mem_cfg->acp_receive_fifo_buffer);
		hs_fifo_addr = (hs_fifo_addr & ACP_DRAM_ADDRESS_MASK);
		hs_fifo_addr = (hs_fifo_addr - ACP_DRAM_PHY_TRNS);
		io_reg_write((PU_REGISTER_BASE + ACP_HS_RX_FIFOADDR), hs_fifo_addr);
		io_reg_write((PU_REGISTER_BASE + ACP_HS_RX_FIFOSIZE),
		...
		hs_buff_addr = (config->elem_array.elems[0].dest | ACP_DRAM_ADDR_TRNS);
		io_reg_write((PU_REGISTER_BASE + ACP_HS_RX_RINGBUFADDR), hs_buff_addr);
		io_reg_write((PU_REGISTER_BASE + ACP_HS_RX_RINGBUFSIZE), hs_buff_size);
		...
		tr_err("Config channel direction undefined %d", channel->direction);
		return -EINVAL;
	}
					...
					uint32_t *avail, uint32_t *free)
{
	if (channel->direction == DMA_DIR_MEM_TO_DEV) {
		...
		*free = (curr_tx_pos - prev_tx_pos) > hs_buff_size ?
		...
		*free = (curr_rx_pos - prev_rx_pos) > hs_buff_size ?
			(curr_rx_pos - prev_rx_pos) % hs_buff_size :
