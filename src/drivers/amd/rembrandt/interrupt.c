...
SOF_DEFINE_UUID("irq-acp", irq_acp_uuid, 0x6533d0eb, 0xb785, 0x4709,
...
static uint32_t acp_irq_get_status(uint32_t index)
{
	...
	if (index > 7)
		return 0;
	if (index == IRQ_NUM_EXT_LEVEL5) {
		acp_intr_stat1 =  (acp_dsp0_intr_stat1_t)
			io_reg_read(PU_REGISTER_BASE + ACP_DSP0_INTR_STAT1);
		if (acp_intr_stat1.bits.audio_buffer_int_stat)
			return acp_irq_status_read(ACP_DSP0_INTR_STAT1);
		else
...
static void acp_irq_mask_int(uint32_t irq)
{
	...
	if (irq == IRQ_NUM_EXT_LEVEL5)
		acp_irq_update_bits(ACP_DSP0_INTR_CNTL1, mask, 0);
	else
...
static void acp_irq_unmask_int(uint32_t irq)
{
	...
	mask = IRQ_INT_MASK(irq);
	if (irq == IRQ_NUM_EXT_LEVEL5)
		acp_irq_update_bits(ACP_DSP0_INTR_CNTL1, mask, mask);
	else
...
{
	dsp_interrupt_routing_ctrl_0_t interrupt_rout_cntl_0;
	dsp_interrupt_routing_ctrl_1_t interrupt_rout_cntl_1;
	/* Routing the particular interrupts to the particular level */
	interrupt_rout_cntl_0 = (dsp_interrupt_routing_ctrl_0_t) io_reg_read(
			(PU_REGISTER_BASE + DSP_INTERRUPT_ROUTING_CTRL_0));
	interrupt_rout_cntl_0.bits.dma_intr_level	= acp_interrupt_level_5;
	interrupt_rout_cntl_0.bits.az_sw_i2s_intr_level	 = acp_interrupt_level_5;
	io_reg_write((PU_REGISTER_BASE + DSP_INTERRUPT_ROUTING_CTRL_0),
					interrupt_rout_cntl_0.u32all);

	interrupt_rout_cntl_1 = (dsp_interrupt_routing_ctrl_1_t) io_reg_read(
			(PU_REGISTER_BASE + DSP_INTERRUPT_ROUTING_CTRL_1));
	interrupt_rout_cntl_1.bits.host_to_dsp_intr1_level = acp_interrupt_level_3;
	interrupt_rout_cntl_1.bits.wov_intr_level	 = acp_interrupt_level_4;
	interrupt_rout_cntl_1.bits.p1_sw_i2s_intr_level = acp_interrupt_level_5;
	io_reg_write((PU_REGISTER_BASE + DSP_INTERRUPT_ROUTING_CTRL_1),
						interrupt_rout_cntl_1.u32all);
}
...
void acp_intr_enable(void)
{
	...
	acp_dsp0_intr_cntl1_t interrupt_cntl1;
	acp_dsp0_intr_stat1_t interrupt_sts1;
	...
	interrupt_sts1.u32all = 0;
	interrupt_sts1.bits.audio_buffer_int_stat = 0x3F;
	io_reg_write((PU_REGISTER_BASE + ACP_DSP0_INTR_STAT1),
					interrupt_sts1.u32all);
	...
	io_reg_write((PU_REGISTER_BASE + ACP_DSP0_INTR_CNTL), interrupt_cntl.u32all);
	interrupt_cntl1 = (acp_dsp0_intr_cntl1_t) io_reg_read((PU_REGISTER_BASE +
							ACP_DSP0_INTR_CNTL1));
	interrupt_cntl1.u32all = 0;
	io_reg_write((PU_REGISTER_BASE + ACP_DSP0_INTR_CNTL1), interrupt_cntl1.u32all);
	interrupt_cntl1.bits.audio_buffer_int_mask       = INTERRUPT_DISABLE;
	io_reg_write((PU_REGISTER_BASE + ACP_DSP0_INTR_CNTL1), interrupt_cntl1.u32all);
...
void acp_intr_disable(void)
{
	...
	acp_dsp0_intr_stat1_t interrupt_status1;
	acp_external_intr_enb_t ext_interrupt_enb;
	acp_dsp0_intr_cntl_t interrupt_cntl;
	acp_dsp0_intr_cntl1_t interrupt_cntl1;
	...
	interrupt_status1 = (acp_dsp0_intr_stat1_t) io_reg_read((PU_REGISTER_BASE +
							ACP_DSP0_INTR_STAT1));
	/* Check and Clear all the Interrupt status bits */
	interrupt_status1.bits.audio_buffer_int_stat	= 0x3F;
	io_reg_write((PU_REGISTER_BASE + ACP_DSP0_INTR_STAT1), interrupt_status1.u32all);
