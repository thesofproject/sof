...
SOF_DEFINE_UUID("acp-sp", acp_sp_uuid, 0x3ac07334, 0x41ce, 0x4447,
...
int acp_dai_sp_dma_start(struct dma_chan_data *channel)
{
	...
	if (channel->direction == DMA_DIR_MEM_TO_DEV) {
		...
		tr_err(&acp_sp_rn_tr, "Start direction not defined %d", channel->direction);
		return -EINVAL;
	}
...
int acp_dai_sp_dma_stop(struct dma_chan_data *channel)
{
	...
	if (channel->direction == DMA_DIR_MEM_TO_DEV) {
		...
		tr_err(&acp_sp_rn_tr, "Stop direction not defined %d", channel->direction);
		return -EINVAL;
	}
	sp_iter = (acp_i2stdm_iter_t)io_reg_read((PU_REGISTER_BASE + ACP_I2STDM_ITER));
	sp_irer = (acp_i2stdm_irer_t)io_reg_read((PU_REGISTER_BASE + ACP_I2STDM_IRER));
			      ...
			      struct dma_sg_config *config)
{
	...
	if (!config->cyclic) {
		tr_err(&acp_sp_rn_tr, "cyclic configurations only supported!");
		return -EINVAL;
	}
	if (config->scatter) {
		tr_err(&acp_sp_rn_tr, "scatter enabled, that is not supported for now!");
		return -EINVAL;
	}
	...
	if (config->direction ==  DMA_DIR_MEM_TO_DEV) {
		...
		tr_err(&acp_sp_rn_tr, "config channel direction undefined %d", channel->direction);
		return -EINVAL;
	}
				 ...
				 uint32_t *avail, uint32_t *free)
{
	if (channel->direction == DMA_DIR_MEM_TO_DEV) {
		...
		*free = (curr_tx_pos - prev_tx_pos) > sp_buff_size_playback ?
			(curr_tx_pos - prev_tx_pos) % sp_buff_size_playback :
			(curr_tx_pos - prev_tx_pos);
		*avail = sp_buff_size_playback - *free;
		...
		*avail = sp_buff_size_capture - *free;
		...
		*free  = (sp_buff_size_capture >> 1);
		...
		tr_err(&acp_sp_rn_tr, "Channel direction not defined %d", channel->direction);
		return -EINVAL;
	}
	return 0;
}
