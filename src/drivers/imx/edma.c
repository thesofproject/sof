					      ...
					      unsigned int req_chan)
{
	...
	tr_dbg("EDMA: channel_get(%d)", req_chan);

	key = k_spin_lock(&dma->lock);
	if (req_chan >= dma->plat_data.channels) {
		k_spin_unlock(&dma->lock, key);
		tr_err("EDMA: Channel %d out of range", req_chan);
		return NULL;
	}
	...
	if (channel->status != COMP_STATE_INIT) {
		k_spin_unlock(&dma->lock, key);
		tr_err("EDMA: Cannot reuse channel %d", req_chan);
		return NULL;
	}
...
static void edma_channel_put(struct dma_chan_data *channel)
{
	 ...
	 */
	tr_info("EDMA: channel_put(%d)", channel->index);
...
{
	tr_info("EDMA: start(%d)", channel->index);
...
static int edma_release(struct dma_chan_data *channel)
{
	...
	tr_info("EDMA: release(%d)", channel->index);
...
{
	/* TODO actually handle pause/release properly? */
	tr_info("EDMA: pause(%d)", channel->index);
...
{
	tr_info("EDMA: stop(%d)", channel->index);
	/* Validate state */
	// TODO: Should we?
			   ...
			   struct dma_sg_config *config)
{
	...
	case DMA_DIR_DEV_TO_MEM:
		...
		break;
	default:
		tr_err("edma_set_config() unsupported config direction");
		return -EINVAL;
	}

	if (!config->cyclic) {
		tr_err("EDMA: Only cyclic configurations are supported!");
		return -EINVAL;
	}
	if (config->scatter) {
		tr_err("EDMA: scatter enabled, that is not supported for now!");
		return -EINVAL;
	}
...
static int edma_probe(struct dma *dma)
{
	...
	if (!dma->chan) {
		tr_err("EDMA: Probe failure, unable to allocate channel descriptors");
		return -ENOMEM;
	}
...
static int edma_remove(struct dma *dma)
{
	...
	if (!dma->chan) {
		tr_err("EDMA: remove called without probe, it's a no-op");
		return 0;
	}
	for (channel = 0; channel < dma->plat_data.channels; channel++) {
			      ...
			      uint32_t *avail, uint32_t *free)
{
	...
	case DMA_DIR_DEV_TO_MEM:
		*avail = ABS(capture_data_size) / 2;
		break;
	default:
		tr_err("edma_get_data_size() unsupported direction %d",
		       channel->direction);
		return -EINVAL;
	}
	return 0;
