				      ...
				      uint32_t direction)
{
	...
	if ((ring->size <= HOST_PAGE_SIZE * (ring->pages - 1)) ||
		...
		/* error buffer size */
		tr_err("ipc_parse_page_descriptors(): error buffer size");
		return -EINVAL;
	}
	...
	if (!elem_array->elems) {
		tr_err("ipc_parse_page_descriptors(): There is no heap free with this block size: %zu",
		       sizeof(struct dma_sg_elem) * ring->pages);
		return -ENOMEM;
	}
				    ...
				    struct sof_ipc_host_buffer *ring)
{
	...
	if (!chan) {
		tr_err("ipc_get_page_descriptors(): chan is NULL");
		return -ENODEV;
	}
...
#endif
	if (ret < 0) {
		tr_err("ipc_get_page_descriptors(): dma_get_attribute() failed");
		goto out;
	}
	...
	if (ret < 0) {
		tr_err("ipc_get_page_descriptors(): dma_set_config() failed");
		goto out;
	}
	...
	if (ret < 0) {
		tr_err("ipc_get_page_descriptors(): dma_start() failed");
		goto out;
	}
			    ...
			    uint32_t *ring_size)
{
	...
	err = ipc_get_page_descriptors(data_host_buffer->dmac,
				       ...
				       ring);
	if (err < 0) {
		tr_err("ipc: get descriptors failed %d", err);
		goto error;
	}
	...
	*ring_size = ring->size;
	...
	if (err < 0) {
		tr_err("ipc: parse descriptors failed %d", err);
		goto error;
	}
