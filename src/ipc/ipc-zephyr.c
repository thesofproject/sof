...
static void ipc_receive_cb(const void *data, size_t len, void *priv)
{
	...
	if (len == INTEL_ADSP_IPC_CB_MSG) {
		...
		struct ipc *ipc = (struct ipc *)priv_data->priv;
...
static void ipc_ept_init(struct ipc *ipc)
{
	...
	ipc_ept_priv_data.priv = (void *)ipc;

	ret = ipc_service_register_endpoint(ipc_dev, &ipc_ept, &ipc_ept_cfg);
	ARG_UNUSED(ret);
}
...
static void ipc_complete(void)
{
	...
	ret = ipc_service_send(&ipc_ept, NULL, INTEL_ADSP_IPC_SEND_DONE);
...
static bool ipc_is_complete(void)
{
	...
	ret = ipc_service_send(&ipc_ept, NULL, INTEL_ADSP_IPC_SEND_IS_COMPLETE);
...
static int ipc_send_message(uint32_t data, uint32_t ext_data)
{
	...
	ret = ipc_service_send(&ipc_ept, &msg, INTEL_ADSP_IPC_SEND_MSG);
...
void ipc_send_message_emergency(uint32_t data, uint32_t ext_data)
{
	...
	ret = ipc_service_send(&ipc_ept, &msg, INTEL_ADSP_IPC_SEND_MSG_EMERGENCY);
