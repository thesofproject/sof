...
int dai_config_dma_channel(struct comp_dev *dev, void *spec_config)
{
	...
	//union ipc4_gateway_attributes *config = ipc_from_dai_config(spec_config);
...
int ipc_dai_data_config(struct comp_dev *dev)
{
	struct dai_data *dd = comp_get_drvdata(dev);
	struct ipc_config_dai *dai = &dd->ipc_config;
	union ipc4_gateway_attributes *dai_config = ipc_from_dai_config(dd->dai_spec_config);
			...
			void *spec_config)
{
	...
	tr_info(&ipc_tr, "ipc_comp_dai_config() dai type = %d index = %d",
		common_config->type, common_config->dai_index);
