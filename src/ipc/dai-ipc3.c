...
int dai_config_dma_channel(struct comp_dev *dev, void *_config)
{
	struct dai_data *dd = comp_get_drvdata(dev);
	struct sof_ipc_dai_config *config = ipc_from_dai_config(_config);
...
int ipc_comp_dai_config(struct ipc *ipc, struct ipc_config_dai *common_config,
			void *spec_config)
{
	struct sof_ipc_dai_config *config = ipc_from_dai_config(spec_config);
