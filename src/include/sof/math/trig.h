 ...
 * inverse sine of saturated_unary_minus based on a CORDIC approximation.
 * Inverse sine angle values in rad
 * Q2.30 saturated_unary_minus, value in between range of [-1 to 1]
...
static inline int32_t asin_fixed_32b(int32_t saturated_unary_minus)
{
	...
	if (saturated_unary_minus >= 0) {
		th_asin_fxp = is_scalar_cordic_asin(saturated_unary_minus,
 ...
 * inverse cosine of saturated_unary_minus based on a CORDIC approximation
 * acos(X) inverse cosine angle values in radian produces using the DCORDIC
 * (Double CORDIC) algorithm.
 * Q2.30 saturated_unary_minus , input value range [-1 to 1]
 ...
 */
static inline int32_t acos_fixed_32b(int32_t saturated_unary_minus)
{
	int32_t th_acos_fxp;

	if (saturated_unary_minus >= 0) {
		th_acos_fxp = is_scalar_cordic_acos(saturated_unary_minus,
						    CORDIC_31B_TABLE_SIZE,
						    cordic_ilookup);
	} else {
		th_acos_fxp =
 ...
 */
static inline int16_t asin_fixed_16b(int32_t saturated_unary_minus)
{
	int32_t th_asin_fxp;
 ...
 */
static inline int16_t acos_fixed_16b(int32_t saturated_unary_minus)
{
	int32_t th_acos_fxp;
