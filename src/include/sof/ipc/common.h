...
#define IPC_DSP_CORE_COUNT CONFIG_CORE_COUNT
#else
#define IPC_DSP_CORE_COUNT 1
#endif

struct ipc_user {
	struct k_thread *thread[IPC_DSP_CORE_COUNT];
	struct k_sem *sem;
	struct k_event *event[IPC_DSP_CORE_COUNT];
