			    ...
			    uint32_t frames)
{
	...
	int samples_to_consume = 0;

	for (int i = 0; i < samples_to_process; ++i)
		cd->input[cd->input_ptr++] = convert_int16_to_float(src[i]);
	...
	while (cd->input_ptr >= cd->num_frames * n_ch) {
		GoogleCtcAudioProcessingProcess(cd->state, cd->input, cd->output + cd->output_ptr,
						cd->num_frames, n_ch);
		cd->output_ptr += cd->num_frames * n_ch;

		cd->input_ptr -= cd->num_frames * n_ch;
		for (int i = 0; i < cd->input_ptr; ++i)
			cd->input[i] = cd->input[i + cd->num_frames * n_ch];
	}

	samples_to_consume = MIN(cd->output_ptr, audio_stream_samples_without_wrap_s16(sink, dest));
	for (int i = 0; i < samples_to_consume; ++i)
		dest[i] = convert_float_to_int16(cd->output[i]);
	dest = audio_stream_wrap(sink, dest + samples_to_consume);
	output_buffers->size += audio_stream_frame_bytes(sink) * samples_to_consume / n_ch;

	cd->output_ptr -= samples_to_consume;
	for (int i = 0; i < cd->output_ptr; ++i)
		cd->output[i] = cd->output[i + samples_to_consume];
}
			    ...
			    uint32_t frames)
{
	...
	for (int i = 0; i < samples_to_process; ++i)
		cd->input[cd->input_ptr++] = convert_int32_to_float(src[i]);
	src = audio_stream_wrap(source, src + samples_to_process);
	input_buffers->consumed += audio_stream_frame_bytes(source) * samples_to_process / n_ch;
	...
	samples_to_consume = MIN(cd->output_ptr, audio_stream_samples_without_wrap_s24(sink, dest));
	for (int i = 0; i < samples_to_consume; ++i)
			    ...
			    uint32_t frames)
{
	...
	samples_to_consume = MIN(cd->output_ptr, audio_stream_samples_without_wrap_s32(sink, dest));
	for (int i = 0; i < samples_to_consume; ++i)
		dest[i] = convert_float_to_int32(cd->output[i]);
	dest = audio_stream_wrap(sink, dest + samples_to_consume);
	output_buffers->size += audio_stream_frame_bytes(sink) * samples_to_consume / n_ch;
...
static int ctc_init(struct processing_module *mod)
{
	...
	buf_size = 2 * cd->num_frames * sizeof(cd->input[0]) * cd->num_channels;
	...
	buf_size = 2 * cd->num_frames * sizeof(cd->output[0]) * cd->num_channels;
