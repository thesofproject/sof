...
/**
 * stft_convert_to_polar() - Convert FFT output to polar format
 * @fft: FFT state containing the FFT output buffer and parameters as Q1.31.
 * @polar_data: Output buffer for polar data, size is half of FFT size.
 *              Magnitude is Q2.30 and phase is Q5.27.
 */
static void stft_convert_to_polar(struct phase_vocoder_fft *fft, struct ipolar32 *polar_data)
{
	int i;

	/* Get magnitude and phase, convert phase from Q3.29 to Q5.27. */
	for (i = 0; i < fft->half_fft_size; i++) {
		sofm_icomplex32_to_polar(&fft->fft_out[i], &polar_data[i]);
		polar_data[i].angle = Q_SHIFT_RND(polar_data[i].angle, 29, 27);
	}
}

/**
 * stft_convert_to_complex() - Convert polar data back to complex format for IFFT
 * @polar_data: Input polar data, size is half of FFT size.
 *              Magnitude is Q2.30 and phase is Q5.27.
 * @fft: FFT state containing the output buffer where complex data will
 *       be stored and parameters as Q1.31.
 */
static void stft_convert_to_complex(struct ipolar32 *polar_data, struct phase_vocoder_fft *fft)
{
	int i;

	/* Convert phase from Q5.27 to Q3.29, and then to (re, im) complex format as Q1.31*/
	for (i = 0; i < fft->half_fft_size; i++) {
		polar_data[i].angle = Q_SHIFT_LEFT(polar_data[i].angle, 27, 29);
...
/**
 * phase_vocoder_normalize_counters - Prevent int32 overflow in frame counters
 *
 * The interpolation fraction depends on (num_output_ifft * speed) mod 2^29.
 * By subtracting a multiple of the fraction cycle period from num_output_ifft
 * (and the corresponding integer amount from num_input_fft), the fraction is
 * preserved exactly while keeping both counters small.
 */
static void phase_vocoder_normalize_counters(struct phase_vocoder_state *state, int32_t speed)
{
	int32_t g;
	int32_t output_period;
	int32_t delta_output;
	int64_t delta_input_scaled;

	if (state->num_output_ifft < (1 << 28))
		return;

	/* The fraction cycle repeats every (2^29 / gcd(speed, 2^29)) output IFFTs */
	g = gcd(speed, 1 << 29);
	output_period = (1 << 29) / g;

	/* Subtract the largest multiple of output_period that fits */
	delta_output = (state->num_output_ifft / output_period) * output_period;
	if (!delta_output)
		return;

	/* delta_output * speed is a multiple of 2^29 by construction,
	 * so the interpolation fraction is exactly preserved.
	 */
	delta_input_scaled = (int64_t)delta_output * speed;

	state->num_output_ifft -= delta_output;
	state->num_input_fft -= (int32_t)(delta_input_scaled >> 29);
}
...
static void phase_vocoder_interpolation_parameters(struct phase_vocoder_comp_data *cd)
{
	...
	int32_t input_frame_num_floor;

	phase_vocoder_normalize_counters(state, cd->state.speed);

	input_frame_num_frac = (int64_t)state->num_output_ifft * cd->state.speed; /* Q31.29 */
	input_frame_num_floor = (int32_t)(input_frame_num_frac >> 29); /* Use floor, not round */
...
void phase_vocoder_reset_for_new_speed(struct phase_vocoder_comp_data *cd)
{
	...
	/* Keep num_input_fft at 1, not 0, to avoid re-entering the first
	 * analysis FFT cold-start path. That path sets angle_delta to the
	 * absolute phase angle (not a delta between frames). For non-unity
	 * speeds, multiple IFFTs are produced per input FFT, and the
	 * absolute angle gets repeatedly accumulated into output_phase
	 * causing random inter-bin phase incoherence and gain variation.
	 *
	 * By setting num_input_fft = 1 the existing polar, angle_delta,
	 * and output_phase state from the last processing step is preserved.
	 * These contain proper phase deltas, so interpolation continues
	 * correctly at the new speed.
	 */
			      ...
			      int num_angles)
{
	...
	for (i = 0; i < num_angles; i++)
		angle_delta_ch[i] = polar_data_ch[i].angle;
}
...
static int stft_do_fft_ifft(const struct processing_module *mod)
{
	...
	if (!state->num_input_fft) {
		for (ch = 0; ch < cd->process_channels; ch++) {
			...
			/* Convert half-FFT to polar. Magnitude is Q2.30 and phase
			 * angle is Q5.27.
			 */
			polar_data_ch = polar->polar[ch]; /* struct ipolar32 */
			stft_convert_to_polar(&state->fft, polar_data_ch);

			/* Initialize output phase directly to the absolute phase
			 * of the first frame. This avoids the accumulation path
			 * which would double-count the initial phase when it
			 * later moves from angle_delta to angle_delta_prev.
			 */
			output_phase_ch = polar->output_phase[ch];
			copy_polar_angles(output_phase_ch, polar_data_ch, fft->half_fft_size);

			/* Initialize prev polar to same data so magnitude
			 * interpolation works before second FFT is consumed.
			 * Leave angle_delta and angle_delta_prev at zero so the
			 * phase does not advance until real deltas are computed.
			 */
			memcpy(polar->polar_prev[ch], polar_data_ch, polar_fft_half_bytes);
		}
		state->num_input_fft++;
		num_fft--;
	}
	...
	while (state->num_input_fft < state->num_input_fft_to_use && num_fft > 0) {
		for (ch = 0; ch < cd->process_channels; ch++) {
			...
			for (i = 0; i < fft->half_fft_size; i++) {
				a = polar_data_ch[i].angle - polar_data_prev_ch[i].angle;
	...
	for (ch = 0; ch < cd->process_channels; ch++) {
		...
		for (i = 0; i < fft->half_fft_size; i++) {
			...
			polar->polar_tmp[i].angle = output_phase_ch[i];
		}
			     ...
			     struct sof_sink *sink, uint32_t source_frames, uint32_t sink_frames)
{
	...
	if (phase_vocoder_check_fft_run_need(cd)) {
		...
		ret = phase_vocoder_source_s32(cd, source, source_frames);
		if (ret)
			return ret;
...
static int phase_vocoder_s24(const struct processing_module *mod, struct sof_source *source,
			     struct sof_sink *sink, uint32_t source_frames, uint32_t sink_frames)
{
	struct phase_vocoder_comp_data *cd = module_get_private_data(mod);
	int ret;
	...
	if (phase_vocoder_check_fft_run_need(cd)) {
		/* Get samples from source buffer */
		ret = phase_vocoder_source_s24(cd, source, source_frames);
		if (ret)
			return ret;
	...
	/* Get samples from source buffer */
	if (cd->state.first_output_ifft_done)
		ret = phase_vocoder_sink_s24(cd, sink, sink_frames);
	else
		/* Use the s32 function */
		ret = phase_vocoder_output_zeros_s32(cd, sink, sink_frames);
			     ...
			     struct sof_sink *sink, uint32_t source_frames, uint32_t sink_frames)
{
	...
	if (phase_vocoder_check_fft_run_need(cd)) {
		/* Get samples from source buffer */
		ret = phase_vocoder_source_s16(cd, source, source_frames);
		if (ret)
			return ret;
...
#endif
#if CONFIG_FORMAT_S24LE
	{SOF_IPC_FRAME_S24_4LE, phase_vocoder_s24},
#endif
#if CONFIG_FORMAT_S32LE
