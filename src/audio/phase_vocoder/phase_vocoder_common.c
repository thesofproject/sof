...
static int stft_do_fft_ifft(const struct processing_module *mod)
{
	...
	if (!state->num_input_fft) {
		for (ch = 0; ch < cd->process_channels; ch++) {
			...
			/* Copy the first phase angle to angle delta */
			angle_delta_ch = polar->angle_delta[ch]; /* int32_t */
			copy_polar_angles(angle_delta_ch, polar_data_ch, fft->half_fft_size);

			/* Initialize prev to same data so interpolation works
			 * correctly before second FFT is consumed.
			 */
			memcpy(polar->polar_prev[ch], polar_data_ch, polar_fft_half_bytes);
			memcpy(polar->angle_delta_prev[ch], angle_delta_ch, int32_fft_half_bytes);
		}
		state->num_input_fft++;
		num_fft--;
	}
