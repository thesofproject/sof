...
static void phase_vocoder_reset_parameters(struct processing_module *mod)
{
	struct phase_vocoder_comp_data *cd = module_get_private_data(mod);

	memset(cd, 0, sizeof(*cd));

	/* TODO: Set enable to false to make sure processing is not enabled if
	 * there is no control provided by tplg.
	 */
	cd->enable = true;
