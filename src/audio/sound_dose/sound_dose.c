...
void sound_dose_report_mel(const struct processing_module *mod)
{
	...
	tmp = cd->total_frames_count & 0x7fffffff;
	cd->feature->stream_time_ms = Q_MULTSR_32X32(tmp, cd->rate_to_ms_coef, 0, 31, 0);
	tmp = cd->total_frames_count >> 31;
	cd->feature->stream_time_ms += tmp * cd->rate_to_ms_coef;
	comp_info(dev, "Time %d dBFS %d MEL %d",
		  (int32_t)cd->feature->stream_time_ms, cd->dose->dbfs_value, cd->dose->mel_value);
...
int sound_dose_filters_init(struct processing_module *mod)
{
	...
	case 48000:
		...
		cd->rate_to_ms_coef = SOUND_DOSE_1K_OVER_48K_Q31;
		break;
	default:
