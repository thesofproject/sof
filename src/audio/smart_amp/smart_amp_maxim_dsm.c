			    ...
			    struct comp_dev *dev)
{
	...
	for (x = 0 ; (x < num_param << 1) ; x++) {
		value[DSM_SET_ID_IDX] = db[x * DSM_PARAM_MAX + DSM_PARAM_ID];
		value[DSM_SET_VALUE_IDX] = db[x * DSM_PARAM_MAX + DSM_PARAM_VALUE];
			      ...
			      int nsamples, int szsample)
{
	...
	if (*w_ptr >= DSM_FF_BUF_SZ) {
		if (is_16bit) {
			...
			for (x = 0; x < DSM_FRM_SZ; x++) {
				input32[x] = buf.buf32[2 * x];
				input32[x + DSM_FRM_SZ] =
					buf.buf32[2 * x + 1];
			}
		}
		...
		*w_ptr -= DSM_FF_BUF_SZ;
		...
		if (is_16bit) {
			...
			for (x = 0; x < DSM_FRM_SZ; x++) {
				buf_out.buf32[*r_ptr + 2 * x] = output32[x];
				buf_out.buf32[*r_ptr + 2 * x + 1] =
					output32[x + DSM_FRM_SZ];
			}
		}
