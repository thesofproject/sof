				   ...
				   enum sof_stft_process_fft_window_type name)
{
	...
	case STFT_BLACKMAN_WINDOW:
		win_blackman_16b(state->window, fft->fft_size, STFT_BLACKMAN_A0);
		return 0;
	case STFT_HAMMING_WINDOW:
		win_hamming_16b(state->window, fft->fft_size);
		return 0;
	case STFT_POVEY_WINDOW:
		       ...
		       int sample_rate, int channels)
{
	...
	state->window = (int16_t *)(state->prev_data + state->prev_data_size);
