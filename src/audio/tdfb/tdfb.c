...
/* The driver assigns running numbers for control index, bytes control has for this
 * component index 0. Enum controls receive indices 1 and 2. Make sure the on/off
 * control is first in topology.
 */
...
static int16_t *tdfb_filter_seek(struct sof_tdfb_config *config, int num_filters)
{
	...
	int i;
	int16_t *coefp = ASSUME_ALIGNED(&config->data[0], 2);
...
static int tdfb_cmd_get_value(struct comp_dev *dev, struct sof_ipc_ctrl_data *cdata)
{
	...
	case SOF_CTRL_CMD_ENUM:
		comp_info(dev, "tdfb_cmd_get_data(), SOF_CTRL_CMD_ENUM index=%d", cdata->index);
		switch (cdata->index) {
...
static int tdfb_copy(struct comp_dev *dev)
{
	...
	/* Update every 1s the the direction angle */
	//cd->count++;
	//if (cd->count > 999) {
	//	cd->count = 0;
	//	cd->az_value++;
