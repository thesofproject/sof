...
#define RTNR_ENABLE_PROCESSING  1

#if RTNR_ENABLE_PROCESSING
#include <sof/audio/rtnr/rtklib/include/Ringbuffer_new.h>
#include <sof/audio/rtnr/rtklib/include/RTK_MA_API.h>
#endif // RTNR_ENABLE_PROCESSING
...

{
	...
	in_source_index = RTNR_MIC_SOURCE_INDEX;
	nch = sources[in_source_index]->channels;

	/* Input from selected input channel, same output to every channel */
	in_idx = cd->source_channel;
	for (i = 0; i < frames; i++) {
		x = audio_stream_read_frag_s16(sources[in_source_index], in_idx);
		mix = *x << 16;
		in_idx += nch;
...
static int rtnr_copy(struct comp_dev *dev)
{
	...
	#if RTNR_ENABLE_PROCESSING
	/* put dummy data into output queue*/
	...
	if (cd->count) {
		...
		for (int i = 0; i < num_sources; ++i)
			comp_info(dev, "rtnr_copy() sources[%d]->id: %d, frames = %d",
				i, sources[i]->id, frames);
		cd->count = 249; // about once per second when 16kHz
	}
