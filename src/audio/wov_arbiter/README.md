...
pauses the other detectors. When sustained silence is detected via the VAD gate kcontrol the arbiter automatically re-arms all detectors for the next trigger; the compress device stays open throughout.
...
| KPB | **Single shared** 192 KB ring buffer (all 3 slots); saves 384 KB vs per-slot design |
| KPB pre-roll (TigerLake) | 6 000 ms — topology-configurable via `KPB_BUFF_TIME_MS` |
...
flowchart LR
    ...
    SWITCH["shared KPB:\nreplay ring buffer at burst rate\nthrough mixin → mixout → wov → arbiter"]
    DRAIN["drain shared ring buffer\n(up to 6 s pre-roll)\narbiter routes slot-N audio to host"]
    LIVE["continue forwarding\nlive DMIC audio\n(all 3 slots share the pre-roll buffer)"]
...
stateDiagram-v2
    ...
    Active --> Idle : trigger(STOP or PAUSE)\nactive_slot = NO_ACTIVE\nbroadcast RESUME\n(pipeline teardown / app exit only — not between-cycle re-arm)
...
The slot index is available as `cd->wov_slot_id` (0, 1, or 2) so each detector
slot can hold independent model state.
...
1. Add `Pipeline 104` (new detect_test slot, no additional KPB needed) following the
   pattern of pipelines 101–103. Route `mixout.104.1 → wov.104.1 → wov-arbiter.105.1`.
   The shared KPB in pipeline 100 automatically feeds the new mixout via mixin.
