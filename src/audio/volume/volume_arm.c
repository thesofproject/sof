...
LOG_MODULE_DECLARE(volume_arm, CONFIG_SOF_LOG_LEVEL);
			   ...
			   struct cir_buf_sink *sink, uint32_t frames, uint32_t attenuation)
{
	...
	if (nch == 2) {
		...
		while (remaining_samples) {
			...
			int i = 0;
			for (; i <= n - 4; i += 4) {
				const uint32_t in0 = *(const uint32_t *)&x[i + 0];
				const uint32_t in1 = *(const uint32_t *)&x[i + 2];
				const int16_t out_l0 = vol_mult_s16((int16_t)in0, vol_l);
				const int16_t out_r0 = vol_mult_s16((int16_t)(in0 >> 16), vol_r);
				const int16_t out_l1 = vol_mult_s16((int16_t)in1, vol_l);
				const int16_t out_r1 = vol_mult_s16((int16_t)(in1 >> 16), vol_r);

				*(uint32_t *)&y[i + 0] = arm_pkhbt((uint16_t)out_l0, (uint16_t)out_r0, 16);
				*(uint32_t *)&y[i + 2] = arm_pkhbt((uint16_t)out_l1, (uint16_t)out_r1, 16);
			}
			for (; i < n; i += 2) {
				const uint32_t in0 = *(const uint32_t *)&x[i + 0];
				const int16_t out_l0 = vol_mult_s16((int16_t)in0, vol_l);
		...
		while (remaining_samples) {
			...
			for (; i <= n - 4; i += 4) {
				const uint32_t in0 = *(const uint32_t *)&x[i + 0];
				const uint32_t in1 = *(const uint32_t *)&x[i + 2];
				const int16_t out0 = vol_mult_s16((int16_t)in0, vol);
				const int16_t out1 = vol_mult_s16((int16_t)(in0 >> 16), vol);
				const int16_t out2 = vol_mult_s16((int16_t)in1, vol);
				const int16_t out3 = vol_mult_s16((int16_t)(in1 >> 16), vol);

				*(uint32_t *)&y[i + 0] = arm_pkhbt((uint16_t)out0, (uint16_t)out1, 16);
				*(uint32_t *)&y[i + 2] = arm_pkhbt((uint16_t)out2, (uint16_t)out3, 16);
			}
			for (; i < n; i++) {
...
#endif
	return arm_sat_s24((int32_t)prod);
}
...
static inline int32_t vol_mult_s32(int32_t x, int32_t vol)
{
	...
	return arm_sat_q31(prod);
#elif COMP_VOLUME_Q1_31
	return arm_smmulr(x, vol);
#else
	const int64_t prod = ((int64_t)x * vol + (1LL << 22)) >> 23;
	return arm_sat_q31(prod);
#endif
