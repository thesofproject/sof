	...
	while (remaining_samples) {
		...
		if (nch == 2) {
			...
			for (; i <= n - 4; i += 4) {
				...
				const int16_t out_l0 = vol_mult_s16(x0, vol_l);
				const int16_t out_r0 = vol_mult_s16(x1, vol_r);
				const int16_t out_l1 = vol_mult_s16(x2, vol_l);
				const int16_t out_r1 = vol_mult_s16(x3, vol_r);
	...
	while (remaining_samples) {
		...
		if (nch == 2) {
			...
			for (; i <= n - 4; i += 4) {
				...
				peak_l = MAX(abs(sign_extend_s24(x0)), peak_l);
				peak_r = MAX(abs(sign_extend_s24(x1)), peak_r);
				peak_l = MAX(abs(sign_extend_s24(x2)), peak_l);
				peak_r = MAX(abs(sign_extend_s24(x3)), peak_r);
			}
			for (; i < n; i += 2) {
				const int32_t x0 = x[i + 0];
				const int32_t x1 = x[i + 1];
			...
			peak_l <<= PEAK_24S_32C_ADJUST;
			peak_r <<= PEAK_24S_32C_ADJUST;
			cd->peak_regs.peak_meter[0] = MAX(peak_l, cd->peak_regs.peak_meter[0]);
			cd->peak_regs.peak_meter[1] = MAX(peak_r, cd->peak_regs.peak_meter[1]);
		} else {
			for (int j = 0; j < nch; j++) {
				...
				for (int i = j; i < n; i += nch) {
					...
					peak = MAX(abs(sign_extend_s24(x[i])), peak);
				}
				peak <<= PEAK_24S_32C_ADJUST;
				cd->peak_regs.peak_meter[j] = MAX(peak, cd->peak_regs.peak_meter[j]);
			}
		}
			       ...
			       struct cir_buf_sink *sink, uint32_t frames, uint32_t attenuation)
{
	...
	while (remaining_samples) {
		...
		if (nch == 2) {
			...
			const int32_t ipeak_l = (int32_t)(peak_l * 2147483647.0f);
			const int32_t ipeak_r = (int32_t)(peak_r * 2147483647.0f);
			...
			for (int j = 0; j < nch; j++) {
				...
				const int32_t ipeak = (int32_t)(peak * 2147483647.0f);
