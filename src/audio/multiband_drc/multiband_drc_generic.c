...
static void multiband_drc_process_emp_crossover_float(struct multiband_drc_state_float *state,
						      crossover_split_float split_func,
						      float *buf_src,
						      float *buf_sink,
						      int enable_emp,
						      int nch,
						      int nband)
{
	struct iir_state_df1_float *emp_s;
	struct crossover_state_float *crossover_s;
	float *buf_sink_band;
	int ch, band;
	float emp_out;
	float crossover_out[nband];
	...
	for (ch = 0; ch < nch; ch++) {
		...
		if (enable_emp)
			emp_out = iir_df1_float(emp_s, *buf_src);
		else
			emp_out = *buf_src;
...
static void multiband_drc_float_process_drc(struct drc_state *state,
					    const struct sof_drc_params *p,
					    float *buf_src,
					    float *buf_sink,
					    int nch)
{
	...
	for (ch = 0; ch < nch; ++ch) {
		pd_write = (float *)state->pre_delay_buffers[ch] + pd_write_index;
		pd_read = (float *)state->pre_delay_buffers[ch] + pd_read_index;
		*pd_write = *buf_src;
		*buf_sink = *pd_read;
	...
	/* Process the input division (32 frames). */
	if (!(pd_write_index & DRC_DIVISION_FRAMES_MASK)) {
		drc_update_detector_average(state, p, 0, nch);
		drc_update_envelope(state, p);
		drc_compress_output(state, p, 0, nch);
	}
}

static void multiband_drc_process_deemp_float(struct multiband_drc_state_float *state,
					      float *buf_src,
					      float *buf_sink,
					      int enable_deemp,
					      int nch,
					      int nband)
{
	struct iir_state_df1_float *deemp_s;
	float *buf_src_band;
	int ch, band;
	float mix_out;
	...
	for (ch = 0; ch < nch; ch++) {
		...
		mix_out = 0.0f;
		for (band = 0; band < nband; band++) {
			mix_out += *buf_src_band;
			buf_src_band += PLATFORM_MAX_CHANNELS;
		}

		if (enable_deemp)
			*buf_sink = iir_df1_float(deemp_s, mix_out);
		else
			*buf_sink = mix_out;
...
static int multiband_drc_float_default(const struct processing_module *mod,
				       struct sof_source *source,
				       struct sof_sink *sink,
				       uint32_t frames)
{
	struct multiband_drc_comp_data *cd = module_get_private_data(mod);
	struct multiband_drc_state_float *state = &cd->state_f;
	float buf_src[PLATFORM_MAX_CHANNELS];
	float buf_sink[PLATFORM_MAX_CHANNELS];
	float buf_drc_src[PLATFORM_MAX_CHANNELS * SOF_MULTIBAND_DRC_MAX_BANDS];
	float buf_drc_sink[PLATFORM_MAX_CHANNELS * SOF_MULTIBAND_DRC_MAX_BANDS];
	float *band_buf_drc_src;
	float *band_buf_drc_sink;
	const int32_t *x, *buf_x_start;
	int32_t *y, *buf_y_start;
	int buf_x_samples, buf_y_samples;
	size_t n_bytes = frames * source_get_frame_bytes(source);
	...
	while (samples) {
		...
		for (i = 0; i < npcm; i += nch) {
			...
			multiband_drc_process_emp_crossover_float(state, cd->crossover_split_f,
								  buf_src, buf_drc_src,
								  enable_emp_deemp, nch, nband);
			...
			band_buf_drc_sink = buf_drc_sink;
			for (band = 0; band < nband; ++band) {
				multiband_drc_float_process_drc(&state->drc[band],
								&cd->config->drc_coef[band],
								band_buf_drc_src, band_buf_drc_sink,
								nch);
				band_buf_drc_src += PLATFORM_MAX_CHANNELS;
			...
			multiband_drc_process_deemp_float(state, buf_drc_sink, buf_sink,
							  enable_emp_deemp, nch, nband);
		...
		samples -= npcm;
		if ((const int32_t *)xf >= buf_x_start + buf_x_samples)
			xf = (const float *)buf_x_start;
		if ((int32_t *)yf >= buf_y_start + buf_y_samples)
			yf = (float *)buf_y_start;
	}
	source_release_data(source, n_bytes);
	sink_commit_buffer(sink, n_bytes);
	return 0;
...
#if CONFIG_FORMAT_FLOAT
	{ SOF_IPC_FRAME_FLOAT, multiband_drc_float_default },
#endif /* CONFIG_FORMAT_FLOAT */
};
...
#if CONFIG_FORMAT_FLOAT
	{ SOF_IPC_FRAME_FLOAT, multiband_drc_default_pass },
#endif /* CONFIG_FORMAT_FLOAT */
};
