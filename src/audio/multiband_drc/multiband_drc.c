				 ...
				 struct sof_sink **sinks, int num_of_sinks)
{
	...
	int sink_frames = sink_get_free_frames(sinks[0]);
	int ret;

	comp_dbg(dev, "entry");

	/* Process only as many frames as both the source provides and the
	 * sink can accept, otherwise the copy would fail with -ENODATA.
	 */
