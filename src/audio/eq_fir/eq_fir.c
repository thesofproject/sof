...
static int eq_fir_setup(struct comp_data *cd, int nch)
{
	...
	cd->fir_delay = rballoc(RZONE_RUNTIME, 0, SOF_MEM_CAPS_RAM, size_sum);
	if (!cd->fir_delay) {
...
static struct comp_dev *eq_fir_new(struct sof_ipc_comp *comp)
{
	 ...
	 */
	if (bs) {
		cd->config = rballoc(RZONE_RUNTIME, 0, SOF_MEM_CAPS_RAM, bs);
		if (!cd->config) {
			rfree(dev);
			rfree(cd);
			return NULL;
			    ...
			    struct sof_ipc_ctrl_data *cdata)
{
	...
	case SOF_CTRL_CMD_BINARY:
		...
		if (cdata->msg_index == 0) {
			...
			cd->config = rballoc(RZONE_RUNTIME, 0, SOF_MEM_CAPS_RAM,
					     cdata->num_elems +
