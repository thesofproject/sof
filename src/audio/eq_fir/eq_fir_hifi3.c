	...
	while (samples) {
		...
		for (ch = 0; ch < nch; ch++) {
			...
			for (i = 0; i < (n >> 1); i += nch) {
				...
				AE_L32_XP(d0, in, inc_nch_s);
				AE_L32_XP(d1, in, inc_nch_s);
				fir_32x16_2x_hifi3(f, d0, d1, out, out1, shift);
				AE_L32_XC(d0, out, inc_2nch_s);
				AE_L32_XC(d1, out1, inc_2nch_s);
			}
		}
		samples -= n;
	...
	while (samples) {
		...
		for (ch = 0; ch < nch; ch++) {
			...
			for (i = 0; i < (n >> 1); i += nch) {
				...
				AE_S32_L_XC(d0, out, inc_nch_s);
				AE_S32_L_XC(d1, out, inc_nch_s);
			}
		}
		samples -= n;
		dst = audio_stream_wrap(sink, dst + n);
		   ...
		   struct output_stream_buffer *bsink, int frames)
{
	...
	while (samples) {
		...
		for (ch = 0; ch < nch; ch++) {
			...
			for (i = 0; i < (n >> 1); i += nch) {
				/* Load two input samples via input pointer x */
				AE_L16_XP(d0, in, inc_nch_s);
				AE_L16_XP(d1, in, inc_nch_s);
				...
				/* Store output and update output pointers */
				AE_S16_0_XC(d0, out, inc_nch_s);
				AE_S16_0_XC(d1, out, inc_nch_s);
			}
		}
		samples -= n;
		dst = audio_stream_wrap(sink, dst + n);
