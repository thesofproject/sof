...
static inline int module_adapter_simple_copy(struct comp_dev *dev)
{
...
static inline int module_adapter_simple_copy_one_source_one_sink(struct processing_module *mod)
{
	struct comp_buffer __sparse_cache *source_c;
	struct comp_buffer __sparse_cache *sink_c;
...
int module_adapter_copy(struct comp_dev *dev)
{
	...
	if (!mod->multi_source_or_sink)
		return module_adapter_simple_copy_one_source_one_sink(mod);
...
int module_adapter_bind(struct comp_dev *dev, void *data)
{
	...
	if (count_ready_or_higher_sources(mod_source_info) > 1) {
		comp_info(dev, "module_adapter_bind(), multi_sink_or_source = true");
		mod->multi_source_or_sink = true;
	}
...
int module_adapter_unbind(struct comp_dev *dev, void *data)
{
	...
	if (mod->simple_copy && count_ready_or_higher_sources(mod_source_info) == 1) {
		comp_info(dev, "module_adapter_unbind(), multi_sink_or_source = false");
		mod->multi_source_or_sink = false;
	}
