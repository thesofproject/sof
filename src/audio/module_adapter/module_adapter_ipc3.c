					 ...
					 bool set)
{
	 ...
	 */
	if (set && drv->adapter_ops->set_configuration)
		return drv->adapter_ops->set_configuration(mod, cdata->data[0].type, pos,
							   data_offset_size, (const uint8_t *)cdata,
							   cdata->num_elems, NULL, 0);
	else if (!set && drv->adapter_ops->get_configuration)
		return drv->adapter_ops->get_configuration(mod, pos, &data_offset_size,
...
int module_adapter_cmd(struct comp_dev *dev, int cmd, void *data, int max_data_size)
{
	...
	case COMP_CMD_SET_VALUE:
		...
		if (drv->adapter_ops->set_configuration)
			ret = drv->adapter_ops->set_configuration(mod, 0,
	...
	case COMP_CMD_GET_VALUE:
		...
		if (drv->adapter_ops->get_configuration)
			ret = drv->adapter_ops->get_configuration(mod, 0, 0, (uint8_t *)cdata, 0);
		break;
	default:
