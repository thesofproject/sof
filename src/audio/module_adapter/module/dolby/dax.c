...
static int dax_buffer_alloc(struct dax_buffer *dax_buff, uint32_t bytes)
{
	...
	dax_buff->addr = rballoc(0, SOF_MEM_CAPS_RAM, bytes);
	if (!dax_buff->addr)
		dax_buff->addr = rzalloc(SOF_MEM_ZONE_RUNTIME, 0, SOF_MEM_CAPS_RAM, bytes);
	if (!dax_buff->addr)
		return -ENOMEM;
...
static int sof_dax_init(struct processing_module *mod)
{
	...
	md->private = rzalloc(SOF_MEM_ZONE_RUNTIME, 0, SOF_MEM_CAPS_RAM, sizeof(struct sof_dax));
	if (!md->private) {
