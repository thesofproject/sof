...
#define PROFILE_MASK 0x2
#define DEVICE_MASK 0x4
#define CP_MASK 0x8
...
static int set_crosstalk_cancellation_enable(struct processing_module *mod, int32_t enable)
{
	...
	comp_info(mod->dev, "set ctc enable %d, ret", enable, ret);
	return ret;
}
				 ...
				 uint32_t id, void *value, uint32_t size)
{
	...
	{
		int32_t enable = *((int32_t *)value);

		enable = !!enable;
		if (dax_ctx->enable != enable) {
			dax_ctx->enable = enable;
			dax_ctx->update_flags |= ENABLE_MASK;
		}
		break;
	}
	case DAX_PARAM_ID_ABSOLUTE_VOLUME:
	{
		...
		dax_ctx->volume = volume;
		dax_ctx->update_flags |= VOLUME_MASK;
		break;
	}
	case DAX_PARAM_ID_OUT_DEVICE:
	{
		int32_t out_device = *((int32_t *)value);

		if (dax_ctx->out_device != out_device) {
			dax_ctx->out_device = out_device;
			dax_ctx->update_flags |= DEVICE_MASK;
		}
		break;
	}
	case DAX_PARAM_ID_PROFILE:
	{
		int32_t profile_id = *((int32_t *)value);

		if (dax_ctx->profile != profile_id) {
			dax_ctx->profile = profile_id;
			dax_ctx->update_flags |= PROFILE_MASK;
		}
		break;
	}
	case DAX_PARAM_ID_CP_ENABLE:
	{
		int32_t content_processing_enable = *((int32_t *)value);

		content_processing_enable = !!content_processing_enable;
		if (dax_ctx->content_processing_enable != content_processing_enable) {
			dax_ctx->content_processing_enable = content_processing_enable;
			dax_ctx->update_flags |= CP_MASK;
		}
		break;
	}
	case DAX_PARAM_ID_CTC_ENABLE:
	{
		int32_t ctc_enable = *((int32_t *)value);

		ctc_enable = !!ctc_enable;
		if (dax_ctx->ctc_enable != ctc_enable) {
			dax_ctx->ctc_enable = ctc_enable;
			dax_ctx->update_flags |= CTC_MASK;
		}
		break;
	}
	case DAX_PARAM_ID_ENDPOINT:
	{
		int32_t endpoint = *((int32_t *)value);

		if (dax_ctx->endpoint == endpoint) {
...
{
	struct comp_dev *dev = mod->dev;
	struct module_param *param = NULL;
	void *pos = data;
	const uint32_t param_header_size = 8;
	uint32_t param_data_size = 0;
...
static void check_and_update_settings(struct processing_module *mod)
{
	...
	if (dax_ctx->update_flags & ENABLE_MASK) {
	...
	if (dax_ctx->update_flags & DEVICE_MASK) {
	...
	if (dax_ctx->update_flags & CTC_MASK) {
		set_crosstalk_cancellation_enable(mod, dax_ctx->ctc_enable);
		dax_ctx->update_flags |= PROFILE_MASK;
		dax_ctx->update_flags &= ~CTC_MASK;
		return;
	}
	if (dax_ctx->update_flags & PROFILE_MASK) {
...
static int sof_dax_init(struct processing_module *mod)
{
	...
	struct sof_dax *dax_ctx = NULL;
	uint32_t persist_sz = 0;
			   ...
			   int num_of_sources, struct sof_sink **sinks, int num_of_sinks)
{
	...
	uint32_t consumed_bytes = 0, processed_bytes = 0, produced_bytes = 0;
