...
static int iadk_modules_init(struct processing_module *mod)
{
	...
	const struct ipc4_base_module_cfg *src_cfg =
				(const struct ipc4_base_module_cfg *)md->cfg.init_data;
	int ret = 0;
	byte_array_t mod_cfg;

	/* FIXME: .init_data is const, the attribute should be preserved */
	mod_cfg.data = (void *)md->cfg.init_data;
