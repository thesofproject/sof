			      ...
			      struct user_worker_data *worker_data)
{
	...
	buffer = rzalloc(SOF_MEM_ZONE_SYS_RUNTIME, 0, SOF_MEM_CAPS_RAM | SOF_MEM_CAPS_MMU_SHD,
			 MAX_PARAM_SIZE * 2);
	if (!buffer)
		return -ENOMEM;
				    ...
				    k_work_user_handler_t user_handler)
{
	...
	if (!wd) {
		wd = rzalloc(SOF_MEM_ZONE_SYS_RUNTIME, 0, SOF_MEM_CAPS_RAM | SOF_MEM_CAPS_MMU_SHD,
			     sizeof(*wd));
		if (!wd)
			return -ENOMEM;
...
{
	const uintptr_t shd_addr = get_shd_heap_start();
	const size_t shd_size = get_shd_heap_size();
			    ...
			    void **adapter)
{
	...
	domain = rzalloc(SOF_MEM_ZONE_SYS_RUNTIME, 0, SOF_MEM_CAPS_RAM, sizeof(*domain));
	if (!domain) {
		ret = -ENOMEM;
		goto error;
	}
 ...
 * @return 0 for success, error otherwise.
 */
static int userspace_module_bind(struct processing_module *mod, void *data)
{
	struct user_worker_data *wr_data = mod->dev_user->wrk_ctx;
	struct module_params *params = (struct module_params *)wr_data->ipc_params;
	int ret;
 ...
 * @return 0 for success, error otherwise.
 */
static int userspace_module_unbind(struct processing_module *mod, void *data)
{
	struct user_worker_data *wr_data = mod->dev_user->wrk_ctx;
	struct module_params *params = (struct module_params *)wr_data->ipc_params;
	int ret;
