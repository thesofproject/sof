...
static int dts_codec_reset(struct processing_module *mod)
{
	...
	comp_dbg(mod->dev, "dts_codec_reset() start");

	dts_result = dtsSofInterfaceReset(codec->private);
	ret = dts_effect_convert_sof_interface_result(mod->dev, dts_result);

	if (ret)
		comp_err(mod->dev, "dts_codec_reset() failed %d %d", ret, dts_result);

	comp_dbg(mod->dev, "dts_codec_reset() done");
			    ...
			    size_t response_size)
{
	...
	ret = dts_codec_apply_config(mod->dev);
	if (ret) {
		comp_err(mod->dev, "dts_codec_set_configuration(): error %x: runtime config apply failed",
			 ret);
		return ret;
	}

	comp_dbg(mod->dev, "dts_codec_set_configuration(): config applied");
