...
static int cadence_codec_init(struct processing_module *mod)
{
	...
	if (codec->state == MODULE_DISABLED && codec->cfg.avail) {
		...
		if (!setup_cfg->data) {
			comp_err(mod->dev, "cadence_codec_init(): failed to alloc setup config");
			ret = -ENOMEM;
			goto free;
		}
		...
		if (ret) {
			comp_err(mod->dev, "cadence_codec_init(): failed to copy setup config %d",
				 ret);
			goto free;
		}
				...
				size_t response_size)
{
	...
	ret = cadence_codec_apply_config(mod->dev);
	if (ret) {
		comp_err(mod->dev, "cadence_codec_set_configuration(): error %x: runtime config apply failed",
			 ret);
		return ret;
	}

	comp_dbg(mod->dev, "cadence_codec_set_configuration(): config applied");
