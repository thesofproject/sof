			      ...
			      size_t response_size)
{
	...
	ret = waves_codec_apply_config(mod->dev);
	if (ret) {
		comp_err(mod->dev, "waves_codec_set_configuration(): error %x: runtime config apply failed",
			 ret);
		return ret;
	}

	comp_dbg(mod->dev, "waves_codec_set_configuration(): config applied");
