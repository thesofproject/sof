...
static int basefw_power_state_info_get(uint32_t *data_offset, char *data)
{
	...
	set_tuple_uint32(tuple, ACTIVE_CORES_MASK, cpu_enabled_cores());
	tuple = next_tuple(tuple);
	...
	set_tuple(tuple, CORE_KCPS, sizeof(core_kcps), core_kcps);
	tuple = next_tuple(tuple);
	*data_offset = (int)((char *)tuple - data);
	return IPC4_SUCCESS;
}
				   ...
				   const char *data)
{
	...
	case IPC4_RESOURCE_ALLOCATION_REQUEST:
		return basefw_resource_allocation_request(first_block, last_block, data_offset, data);
	default:
		break;
	}
