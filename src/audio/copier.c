...
static int copier_params(struct comp_dev *dev, struct sof_ipc_stream_params *params)
{
	...
	comp_err(dev, "dai %p, host %p", cd->dai, cd->host);
	if (cd->host) {
	...
	if (ret < 0)
		comp_err(dev, "failed to hw_params %d", ret);
	else
		comp_err(dev, " to hw_params %d", ret);
		      ...
		      int max_data_size)
{
	...
	case IPC4_MODULE_BIND:
	case IPC4_MODULE_UNBIND:
		return 0;
	default:
		return -EINVAL;
	}
