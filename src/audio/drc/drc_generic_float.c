...
static float volume_gain(const struct sof_drc_params *p, float x)
{
	...
	if (x < knee_threshold) {
		if (x < linear_threshold)
			return 1;
		y = knee_curveK(p, x) * warp_inv(x, 31, 20);
		//float golden = 1.0 / x;
		//float fixed = warp_inv(x, 31, 20);
		//if (fabs(golden - fixed) >= 0.1) {
		//	int32_t xf = Q_CONVERT_FLOAT(x, 31);
		//	fprintf(stderr, "warp_inv error=%f (x=%f(0x%x) g=%f f=%f)\n", fabs(golden - fixed), x, xf, golden, fixed);
		//}
		//y = knee_curveK(p, x) / x;
	} else {
...
static void drc_update_envelope(struct drc_state *state, const struct sof_drc_params *p)
{
	...
	if (is_releasing) {
		...
		db_per_frame = kSpacingDb * warp_inv(release_frames, 12, 30);
		...
		x = 0.25f * warp_inv(eff_atten_diff_db, 22, 26);
