...
{
	const int32_t knee_threshold = arm_sat_q31(((int64_t)p->knee_threshold) << 7);
	const int32_t linear_threshold = arm_sat_q31(((int64_t)p->linear_threshold) << 1);
	int32_t exp_knee;
	int32_t y;
	int32_t tmp;
	int32_t tmp2;
...
void drc_update_envelope(struct drc_state *state, const struct sof_drc_params *p)
{
	...
	if (is_releasing) {
		...
		release_frames = arm_sat_q31((rel64 + 32) >> 6);
