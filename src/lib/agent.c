...
static enum task_state validate(void *data)
{
	...
	if (delta > sa->warn_timeout)
		...
		trace_sa_warn("validate(), ll drift detected, delta = %llu",
			      delta);
#else
		trace_sa_warn("validate(), ll drift detected, delta = %u to %u",
...
void sa_init(struct sof *sof, uint64_t timeout)
{
	...
	trace_sa_error("sa_init(), timeout = %u", (uint32_t)timeout);
...
#endif
	trace_sa_error("sa_init(), ticks = %u, sof->sa->warn_timeout = %u, sof->sa->panic_timeout = %u",
