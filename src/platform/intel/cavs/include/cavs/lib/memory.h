...
static inline void __sparse_cache *platform_uncache_to_cache(void *address)
{
	return (void __sparse_cache *)((uintptr_t)(address) | SRAM_ALIAS_OFFSET);
}

static inline void *platform_cache_to_uncache(void __sparse_cache *address)
{
	return (void *)((uintptr_t)(address) & ~SRAM_ALIAS_OFFSET);
}

#define is_uncached(address) (((uint32_t)(address) & SRAM_ALIAS_MASK) == SRAM_ALIAS_BASE)
#else
