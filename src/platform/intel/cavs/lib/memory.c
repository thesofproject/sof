...
void platform_init_memmap(struct sof *sof)
{
	...
	/*
	 * by default, use cached address for system and runtime zones,
	 * and uncached address for system_runtime and buffer zones.
	 */
	...
	sof->memory_map->system[0].caps = SOF_MEM_CAPS_RAM | SOF_MEM_CAPS_HP |
	...
	sof->memory_map->system_runtime[0].caps = SOF_MEM_CAPS_RAM | SOF_MEM_CAPS_HP |
						  SOF_MEM_CAPS_CACHE | SOF_MEM_CAPS_DMA;
	...
	for (i = 1; i < CONFIG_CORE_COUNT; i++) {
		...
		sof->memory_map->system[i].caps = SOF_MEM_CAPS_RAM | SOF_MEM_CAPS_HP |
						  SOF_MEM_CAPS_CACHE | SOF_MEM_CAPS_DMA;
		...
		sof->memory_map->system_runtime[i].info.free =
			HEAP_SYS_RUNTIME_S_SIZE;
		sof->memory_map->system_runtime[i].caps = SOF_MEM_CAPS_RAM | SOF_MEM_CAPS_HP |
							  SOF_MEM_CAPS_CACHE | SOF_MEM_CAPS_DMA;
	}

	/* .runtime init*/
	sof->memory_map->runtime.blocks = ARRAY_SIZE(rt_heap_map);
	sof->memory_map->runtime.map = uncached_block_map(rt_heap_map);
	sof->memory_map->runtime.heap = (uintptr_t)&_module_heap;
	sof->memory_map->runtime.size = HEAP_RUNTIME_SIZE;
	sof->memory_map->runtime.info.free = HEAP_RUNTIME_SIZE;
	sof->memory_map->runtime.caps = SOF_MEM_CAPS_RAM | SOF_MEM_CAPS_HP |
					SOF_MEM_CAPS_CACHE | SOF_MEM_CAPS_DMA;
	...
	sof->memory_map->buffer[1].heap = cache_to_uncache((uintptr_t)HEAP_LP_BUFFER_BASE);
