...
#define TEENSY41_SAI_CLOCK_MODE SOF_DAI_FMT_CBP_CFP
#else
/* Board A: Master Tx (generates BCLK and SYNC on Pin 21 and Pin 20) */
#define TEENSY41_SAI_CLOCK_MODE SOF_DAI_FMT_CBC_CFC
#endif
...
static const uint32_t teensy41_default_iir_coef_2ch[51] = {
...
static const uint32_t teensy41_default_drc_coef[35] = {
 ...
 * ------------------------------------------------------------------------- */
static const struct sof_static_comp teensy41_comps[] = {
	...
	SOF_STATIC_COMP_MODULE(
		...
		.init_blob = teensy41_default_iir_coef_2ch,
		.init_blob_size = sizeof(teensy41_default_iir_coef_2ch)
	),
	SOF_STATIC_COMP_MODULE(
	...
	SOF_STATIC_COMP_DAI(
		...
		.uuid = &dai_uuid, .direction = SOF_IPC_STREAM_PLAYBACK,
		.caps = SOF_STATIC_CAPS(SOF_IPC_FRAME_S16_LE, 48000, 2),
		.ep.dai.dai_type = SOF_DAI_IMX_SAI,
		.ep.dai.dai_index = 1,
 ...
 * ------------------------------------------------------------------------- */
static const struct sof_static_buffer teensy41_buffers[] = {
	/* Direct Playback Buffer: USB_PB (1) -> [1] -> DAI_I2S (5) */
	SOF_STATIC_BUFFER(.id = 1, .size = 3072, .fmt = SOF_IPC_FRAME_S16_LE),

	/* Direct Capture Buffer: DAI_I2S (6) -> [5] -> USB (10) */
 ...
 * ------------------------------------------------------------------------- */
static const struct sof_static_route teensy41_routes[] = {
	/* Direct Playback Route: USB_PB (1) -> [1] -> DAI_I2S (5) */
	SOF_STATIC_ROUTE(.src_comp_id = 1, .buffer_id = 1, .sink_comp_id = 5),

	/* Direct Capture Route: DAI_I2S (6) -> [5] -> USB (10) */
	SOF_STATIC_ROUTE(.src_comp_id = 6, .buffer_id = 5, .sink_comp_id = 10),
};

/* -------------------------------------------------------------------------
 * 4. PCMs (Pulse Code Modulated Endpoints)
 * ------------------------------------------------------------------------- */
static const struct sof_static_pcm teensy41_pcms[] = {
	SOF_STATIC_PCM(
		.pcm_id = 0,
 ...
 * ------------------------------------------------------------------------- */
static const struct sof_static_kcontrol teensy41_controls[] = {
 ...
 * ------------------------------------------------------------------------- */
static const struct sof_static_pipeline_desc teensy41_pipelines[] = {
	{
		.pipeline_id = 1,
 ...
 * ------------------------------------------------------------------------- */
const struct sof_static_topology g_teensy41_static_topology = {
	.name = "Teensy 4.1 Playback & Capture Pipeline",
	.num_pipelines = ARRAY_SIZE(teensy41_pipelines),
	.pipelines = teensy41_pipelines,
	.num_comps = ARRAY_SIZE(teensy41_comps),
	.comps = teensy41_comps,
	.num_buffers = ARRAY_SIZE(teensy41_buffers),
	.buffers = teensy41_buffers,
	.num_routes = ARRAY_SIZE(teensy41_routes),
	.routes = teensy41_routes,
	.num_pcms = ARRAY_SIZE(teensy41_pcms),
	.pcms = teensy41_pcms,
	.num_controls = ARRAY_SIZE(teensy41_controls),
	.controls = teensy41_controls,
};
