...
typedef struct  acp_scratch_memory_config {
	...
	/* Fifo buffers are not part of scratch memory on ACP_6_3 */
	/* Added fifo members to align with Driver structure */
	/* ACP transmit fifo buffer */
	uint8_t			acp_transmit_fifo_buffer[256] __attribute__((aligned(128)));

	/* ACP receive fifo buffer */
	uint8_t			acp_receive_fifo_buffer[256] __attribute__((aligned(128)));
