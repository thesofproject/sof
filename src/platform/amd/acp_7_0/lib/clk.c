...
void acp_7_0_get_boot_ref_clock(float *boot_ref_clk);
...
void acp_7_0_reg_wait(void)
{
	...
	for (test_count = 0; test_count < 255; test_count++) {
		(void)acp_reg_read_via_smn(CLK7_CLK_FSM_STATUS, sizeof(int));
	}
}

void acp_7_0_get_boot_ref_clock(float *boot_ref_clk)
{
	clk7_clk_pll_refclk_startup_t boot_ref_clk_startup;
	...
	boot_ref_clk_startup.u32all =
		acp_reg_read_via_smn(CLK7_CLK_PLL_REFCLK_RATE_STARTUP, sizeof(int));

	clk7_spll_field_9_t  clk_spll_field_9;

	clk_spll_field_9.u32all = 0x0;
	clk_spll_field_9.u32all = acp_reg_read_via_smn(CLK7_SPLL_FIELD_9, sizeof(int));
	...
	clk7_clk_pll_req.u32all = acp_reg_read_via_smn(CLK7_CLK_PLL_REQ, sizeof(int));
	clk7_spll_field.u32all = acp_reg_read_via_smn(CLK7_SPLL_FIELD_2, sizeof(int));

	spinediv = (1 << clk7_clk_pll_req.bitfields.pllspinediv);

	if (clk7_spll_field.bitfields.spll_fracn_en == 1)
		fract_part = (float)(clk7_clk_pll_req.bitfields.fbmult_frac / (float)65536.0f);

	*boot_ref_clk = (float)(((final_refclk) * (clk7_clk_pll_req.bitfields.fbmult_int +
					fract_part)) / (float)spinediv);
}

void acp_change_clock_notify(uint32_t clock_freq)
{
	volatile clk7_clk1_dfs_cntl_u_t dfs_cntl;
	volatile clk7_clk1_bypass_cntl_u_t bypass_cntl;
	volatile clk7_clk1_dfs_status_u_t dfs_status;
	volatile uint32_t updated_clk = 0;
	float did, fraction_val, boot_ref_clk;
	acp_clock_type_t clock_type = acp_aclk_clock;

	acp_7_0_get_boot_ref_clock(&boot_ref_clk);

	tr_info(&acp_clk_tr, "clock_freq : %d clock_type : %d",
		clock_freq, clock_type);

	fraction_val = (float)(clock_freq / 1000000.0f);
	clock_freq /= 1000000;
	if (acp_aclk_clock == clock_type) {
		bypass_cntl.u32all = acp_reg_read_via_smn(CLK7_CLK1_BYPASS_CNTL, sizeof(int));
		dfs_cntl.u32all = acp_reg_read_via_smn(CLK7_CLK1_DFS_CNTL, sizeof(int));
	} else if (acp_sclk_clock == clock_type) {
		bypass_cntl.u32all = acp_reg_read_via_smn(CLK7_CLK0_BYPASS_CNTL, sizeof(int));
		dfs_cntl.u32all = acp_reg_read_via_smn(CLK7_CLK0_DFS_CNTL, sizeof(int));
	}
	...
	if (acp_aclk_clock == clock_type) {
		acp_reg_write_via_smn(CLK7_CLK1_BYPASS_CNTL, bypass_cntl.u32all, sizeof(int));
		tr_info(&acp_clk_tr, "check CLK7_CLK0_DFS_CNTL : not entered the condition");
		acp_reg_write_via_smn(CLK7_CLK1_DFS_CNTL, dfs_cntl.u32all, sizeof(int));
		dfs_status.u32all = acp_reg_read_via_smn(CLK7_CLK1_DFS_STATUS, sizeof(int));
		acp_7_0_reg_wait();
		...
		updated_clk = acp_reg_read_via_smn(CLK7_CLK1_CURRENT_CNT, sizeof(int));
		acp_7_0_reg_wait();
		...
		if (updated_clk < (clock_freq * 10)) {
			...
			do {
				dfs_status.u32all =
					acp_reg_read_via_smn(CLK7_CLK1_DFS_STATUS,
							     sizeof(int));
				dfs_cntl.u32all =
					acp_reg_read_via_smn(CLK7_CLK1_DFS_CNTL,
							     sizeof(int));
		...
		acp_reg_write_via_smn(CLK7_CLK0_BYPASS_CNTL, bypass_cntl.u32all, sizeof(int));
		tr_info(&acp_clk_tr, "check CLK7_CLK0_DFS_CNTL : %d",dfs_cntl.u32all);
		acp_reg_write_via_smn(CLK7_CLK0_DFS_CNTL, dfs_cntl.u32all, sizeof(int));
		dfs_status.u32all = acp_reg_read_via_smn(CLK7_CLK0_DFS_STATUS, sizeof(int));
		acp_7_0_reg_wait();
		...
		updated_clk = acp_reg_read_via_smn(CLK7_CLK0_CURRENT_CNT, sizeof(int));
	}
	tr_info(&acp_clk_tr,
...
void audio_pll_power_off(void)
{
	...
	for (count = 0; count < 10; count++) {
		clk7_clk_pll_pwr_req.u32all =
			acp_reg_read_via_smn(CLK7_CLK_PLL_PWR_REQ, sizeof(int));
		clk7_clk_pll_pwr_req.bitfields.PLL_AUTO_STOP_REQ = 1;

		acp_reg_write_via_smn(CLK7_CLK_PLL_PWR_REQ,
				      clk7_clk_pll_pwr_req.u32all, sizeof(int));
		acp_7_0_reg_wait();

		clk7_clk_fsm_status.u32all = acp_reg_read_via_smn(CLK7_CLK_FSM_STATUS, sizeof(int));

		if (clk7_clk_fsm_status.bitfields.RO_FSM_PLL_STATUS_STOPPED == 1)
			break;
	}
}
...
void audio_pll_power_on(void)
{
	...
	for (count = 0; count < 10; count++) {
		clk7_clk_pll_pwr_req.u32all =
			acp_reg_read_via_smn(CLK7_CLK_PLL_PWR_REQ, sizeof(int));
		clk7_clk_pll_pwr_req.bitfields.PLL_AUTO_START_REQ = 1;
		...
		clk7_clk_fsm_status.u32all = acp_reg_read_via_smn(CLK7_CLK_FSM_STATUS, sizeof(int));
		if (clk7_clk_fsm_status.bitfields.RO_FSM_PLL_STATUS_STARTED == 1)
			break;
		acp_7_0_reg_wait();
	}
}
...
void clk_dfs_bypassexit(void)
{
	volatile clk7_clk_dfsbypass_cntl_t clk7_clk_dfsbypass_cntl;

	clk7_clk_dfsbypass_cntl = (clk7_clk_dfsbypass_cntl_t)acp_reg_read_via_smn
				(CLK7_CLK_DFSBYPASS_CONTROL, sizeof(int));

	clk7_clk_dfsbypass_cntl.bitfields.exit_dfs_bypass_0 = 1;
	clk7_clk_dfsbypass_cntl.bitfields.exit_dfs_bypass_1 = 1;

	acp_reg_write_via_smn(CLK7_CLK_DFSBYPASS_CONTROL,
			      clk7_clk_dfsbypass_cntl.u32all,
			      sizeof(int));
}
...
void audio_pll_mode_switch(uint32_t mode, uint32_t fcw_int, uint32_t fcw_frac, uint32_t fcw_denom,
			   uint32_t pllspinediv)
{
	volatile clk7_spll_fuse_1_t  clk_spll_fuse1;
	volatile clk7_spll_fuse_2_t  clk_spll_fuse2;
	volatile clk7_spll_field_9_t  clk_spll_field_9;
	volatile clk7_spll_field_6nm_t  clk_spll_field_6nm;
	volatile clk7_spll_field_7_t  clk_spll_field_7;
	volatile clk7_spll_field_4_t  clk_spll_field_4;
	volatile clk7_spll_field_5nm_bus_ctrl_t  clk_spll_field_5nm_bus_ctrl;
	volatile clk7_spll_field_5nm_bus_wdata_t  clk_spll_field_5nm_bus_wdata;
	volatile clk7_spll_field_5nm_bus_status_t  clk_spll_field_5nm_bus_status;
	volatile clk7_rootrefclk_mux_1_t clk_rootrefclkmux;
	volatile clk7_spll_field_2_t  clk7_spll_field_2;
	...
	clk7_spll_field_2 =
		(clk7_spll_field_2_t)acp_reg_read_via_smn(CLK7_SPLL_FIELD_2, sizeof(int));

	if (clk7_spll_field_2.bitfields.spll_fracn_en == 0)
		clk7_spll_field_2.bitfields.spll_fracn_en = 1;
	acp_reg_write_via_smn(CLK7_SPLL_FIELD_2, clk7_spll_field_2.u32all, sizeof(int));

	switch (mode) {
	case PLL_MODE_32KHZ_LPPM:
		clk_rootrefclkmux.bitfields.ROOTREFCLK_MUX_1 = 0x0;
		clk_rootrefclkmux.u32all = acp_reg_read_via_smn(CLK7_ROOTREFCLK_MUX_1, sizeof(int));

		clk_rootrefclkmux.bitfields.ROOTREFCLK_MUX_1    = 1;
		acp_reg_write_via_smn(CLK7_ROOTREFCLK_MUX_1, clk_rootrefclkmux.u32all, sizeof(int));
		...
		acp_reg_write_via_smn(CLK7_SPLL_FUSE_1, clk_spll_fuse1.u32all, sizeof(int));
		acp_reg_write_via_smn(CLK7_SPLL_FUSE_2, clk_spll_fuse2.u32all, sizeof(int));
		acp_reg_write_via_smn(CLK7_SPLL_FIELD_9, clk_spll_field_9.u32all, sizeof(int));
		acp_reg_write_via_smn(CLK7_SPLL_FIELD_6nm, clk_spll_field_6nm.u32all, sizeof(int));
		acp_reg_write_via_smn(CLK7_SPLL_FIELD_7, clk_spll_field_7.u32all, sizeof(int));
		acp_reg_write_via_smn(CLK7_SPLL_FIELD_4, clk_spll_field_4.u32all, sizeof(int));

		clk_spll_field_5nm_bus_wdata.bitfields.bus_spll_wr_data = 0x00400000;
		acp_reg_write_via_smn(CLK7_SPLL_FIELD_5nm_BUS_WDATA,
				      clk_spll_field_5nm_bus_wdata.u32all,
				      sizeof(int));

		clk_spll_field_5nm_bus_ctrl.u32all =
			acp_reg_read_via_smn(CLK7_SPLL_FIELD_5nm_BUS_CTRL,
					     sizeof(int));
		...
		acp_reg_write_via_smn(CLK7_SPLL_FIELD_5nm_BUS_CTRL,
				      clk_spll_field_5nm_bus_ctrl.u32all,
				      sizeof(int));
		do {
			clk_spll_field_5nm_bus_status.u32all =
				acp_reg_read_via_smn(CLK7_SPLL_FIELD_5nm_BUS_STATUS,
						     sizeof(int));
		} while (clk_spll_field_5nm_bus_status.bitfields.spll_bus_rd_valid !=
			 clk_spll_field_5nm_bus_ctrl.bitfields.bus_spll_rdtr);

		acp_reg_write_via_smn(CLK7_CLK_PLL_RESET_STOP_TIMER, 0xbbb11aa, sizeof(int));
		break;
	default:
	...
	clk7_clk_pll_req =
		(clk7_clk_pll_req_u_t)acp_reg_read_via_smn(CLK7_CLK_PLL_REQ, sizeof(int));
	clk7_clk_pll_req.bitfields.fbmult_int   = fcw_int;

	if (clk7_spll_field_2.bitfields.spll_fracn_en)
		clk7_clk_pll_req.bitfields.fbmult_frac  = fcw_frac;
	clk7_clk_pll_req.bitfields.pllspinediv  = pllspinediv;
	acp_reg_write_via_smn(CLK7_CLK_PLL_REQ, clk7_clk_pll_req.u32all, sizeof(int));
}
...
void clk7_init_vco(void)
{
	...
	clk_dfs_bypassexit();
	acp_reg_write_via_smn(CLK7_CLK1_BYPASS_CNTL, 0, sizeof(int));
}
...
void platform_clock_init(struct sof *sof)
{
	...
	acp_reg_write_via_smn(CLK7_CLK0_DFS_CNTL, 0x19, sizeof(int));
}
