...
#
# This makes sof.ri non-deterministic even when sof.elf is
...
# For a list of (non-tested) variations check the "Achieve deterministic
# builds" list found at https://reproducible-builds.org/docs/
...
# Starting from TGL the rimage signature has a random salt (on top of
# the existing date). Longer term we need to stop comparing .ri files
# and support stripping the signature for comparison purposes, see
# "un-rimage" ticket
# https://github.com/thesofproject/rimage/issues/41
