...
def main():
	...
	# critical functions in a .text.dram ELF section, read-only data in a
	# .rodata.dram ELF section, etc. When compiling and
	# linking such functions, additional sections like .literal.dram are
	# automatically created. Note, that for some reason for .text.dram the
	# compiler also marks that section as executable.
