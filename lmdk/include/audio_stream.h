...
static inline uint32_t audio_stream_get_buffer_fmt(struct audio_stream  *buf)
{
					   ...
					   uint32_t bytes)
{
	...
	dcache_invalidate_region((__sparse_force void  *)buffer->r_ptr, head_size);
	if (tail_size)
		dcache_invalidate_region((__sparse_force void  *)buffer->addr,
					 tail_size);
}
					  ...
					  uint32_t bytes)
{
	...
	dcache_writeback_region((__sparse_force void  *)buffer->w_ptr, head_size);
	if (tail_size)
		dcache_writeback_region((__sparse_force void  *)buffer->addr,
					tail_size);
}
